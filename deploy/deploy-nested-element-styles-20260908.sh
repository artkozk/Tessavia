...
[[ "$RELEASE_NAME" =~ ^20260908-nested-element-styles-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-nested-element-styles
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-nested-element-styles-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
release_created=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-nested-element-styles-check.* ]]; then rm -rf -- "$dry"; fi
...
with sqlite3.connect('file:'+sys.argv[1]+'?mode=ro',uri=True) as db:
    for table in ('page_app_definitions','page_app_templates'):
        for row in db.execute('SELECT definition_json FROM '+table):
            if any(bool(block.get('elementStyles')) for block in json.loads(row[0]).get('blocks',[])):
                sys.exit(2)
PYROLLBACK
    then
    systemctl stop business-control || true
    ...
    printf 'ROLLED_BACK=%s\n' "$previous" >&2
      ...
      systemctl start business-control
      printf 'ROLLBACK_BLOCKED=saved_nested_element_styles_require_compatible_binary\n' >&2
    fi
...
dry=$(mktemp -d /tmp/tessavie-nested-element-styles-check.XXXXXX)
...
python3 /tmp/verify-nested-element-styles-migration.py "$backup/business-control.db" "$dry/check.db"

check_http() {
  local base="$1"
  curl --max-time 15 -fsS "$base/" > "$dry/index.html"
  grep -qF 'app.js?v=20260908-nested-element-styles-2' "$dry/index.html"
  grep -qF 'styles.css?v=20260908-nested-element-styles-2' "$dry/index.html"
  curl --max-time 15 -fsS "$base/app.js?v=20260908-nested-element-styles-2" > "$dry/app.js"
  grep -qF 'chat-workspace.js?v=20260908-nested-element-styles-2' "$dry/app.js"
  ...
  grep -qF 'page-apps.js?v=20260908-nested-element-styles-2' "$dry/app.js"
  curl --max-time 15 -fsS "$base/page-apps.js?v=20260908-nested-element-styles-2" > "$dry/page-apps.js"
  curl --max-time 15 -fsS "$base/page-record-bindings.js?v=20260908-nested-element-styles-2" > "$dry/bindings.js"
  grep -qF 'recordBindingText' "$dry/bindings.js"
  grep -qF 'data-binding-remove' "$dry/bindings.js"
  curl --max-time 15 -fsS "$base/page-element-styles.js?v=20260908-nested-element-styles-2" > "$dry/elements.js"
  ...
  curl --max-time 15 -fsS "$base/page-block-visibility.js?v=20260908-nested-element-styles-2" > "$dry/visibility.js"
  ...
  curl --max-time 15 -fsS "$base/page-forms.js?v=20260908-nested-element-styles-2" > "$dry/page-forms.js"
  ...
  curl --max-time 15 -fsS "$base/page-record-actions.js?v=20260908-nested-element-styles-2" > "$dry/page-record-actions.js"
  ...
  curl --max-time 15 -fsS "$base/page-action-conditions.js?v=20260908-nested-element-styles-2" > "$dry/page-action-conditions.js"
  ...
  curl --max-time 15 -fsS "$base/field-conflicts.js?v=20260908-nested-element-styles-2" > "$dry/field-conflicts.js"
  grep -qF 'reviewFieldConflict' "$dry/field-conflicts.js"
  grep -qF 'fieldEditBaseline' "$dry/app.js"
  curl --max-time 15 -fsS "$base/page-data-sources.js?v=20260908-nested-element-styles-2" > "$dry/data.js"
...
PYBOOK
  ...
  curl --max-time 15 -fsS "$base/chat-workspace.js?v=20260908-nested-element-styles-2" > "$dry/chat.js"
  grep -qF 'createChatWorkspaceUI' "$dry/chat.js"
  curl --max-time 15 -fsS "$base/life-map.js?v=20260908-nested-element-styles-2" > "$dry/life.js"
  grep -qF 'life-map-overview' "$dry/life.js"
  curl --max-time 15 -fsS "$base/chat-groups.js?v=20260908-nested-element-styles-2" > "$dry/groups.js"
  grep -qF 'createChatGroupUI' "$dry/groups.js"
  curl --max-time 15 -fsS "$base/personal-calendar.js?v=20260908-nested-element-styles-2" > "$dry/calendar.js"
  grep -qF 'openRecurrence' "$dry/calendar.js"
  grep -qF 'bindPersonalRecurrenceScope' "$dry/app.js"
  curl --max-time 15 -fsS "$base/personal-today.js?v=20260908-nested-element-styles-2" > "$dry/today.js"
  ...
  curl --max-time 15 -fsS "$base/styles.css?v=20260908-nested-element-styles-2" > "$dry/styles.css"
  grep -qF -- '--chat-menu-left' "$dry/styles.css"
  curl --max-time 15 -fsS "$base/sw.js" > "$dry/sw.js"
  grep -qF 'tessavie-shell-20260908-nested-element-styles-2' "$dry/sw.js"

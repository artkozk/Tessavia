import { conditionConfig, updateConditionProperty, conditionMatches, conditionDescription } from './page-action-conditions.js?v=20260908-copy-field-1';
...
export function recordActionConfig(block,collection,e,fieldInput){
 ...
 return `<details class="app-source-schema" ${(block.actions||[]).length?'open':''}><summary>Действия с записью · ${(block.actions||[]).length}</summary><p class="muted">Кнопка изменит поле записи по заданному правилу. Перед изменением человек увидит результат и подтвердит его.</p>${(block.actions||[]).map(a=>{
  const field=fields.find(f=>f.id===a.fieldId),sources=compatibleActionSourceFields(collection,field),mode=a.operation||'set';
  ...
  const value=mode==='copy'?`<label>Источник значения<select data-action-source-field><option value="">Выберите поле</option>${sources.map(f=>`<option value="${f.id}" ${f.id===a.sourceFieldId?'selected':''}>${e(f.name)}</option>`).join('')}</select></label><p class="muted">${sources.length?'Берётся актуальное значение этой же записи. Пустой источник очистит необязательное поле после подтверждения; обязательное поле не очищается.':'На этой доске нет другого совместимого поля. Добавьте поле подходящего типа или выберите другой способ изменения.'}</p>`:field?fieldInput({...field,name:mode==='add'?'Сколько прибавить':'Новое значение',required:mode==='add'||field.required},a.value):'<p class="muted">Выберите доступное поле.</p>';
  return `<section class="app-action-config" data-record-action="${e(a.id)}"><label>Подпись действия<input data-action-label value="${e(a.label)}" maxlength="80"></label><label>Изменяемое поле<select data-action-field><option value="">Выберите поле</option>${fields.map(f=>`<option value="${f.id}" ${f.id===a.fieldId?'selected':''}>${e(f.name)}</option>`).join('')}</select></label>${operation}<div data-action-value>${value}</div>${mode==='add'?'<p class="muted">Отрицательное число уменьшит значение. Пустое поле сначала нужно заполнить; для нового счётчика задайте начальное значение 0.</p>':''}${conditionConfig(a,collection,e,fieldInput)}<button type="button" class="text-button" data-action-remove>Убрать действие</button></section>`;
 }).join('')}<button type="button" class="secondary" data-action-add ${fields.length&&(block.actions||[]).length<8?'':'disabled'}>Добавить действие</button>${fields.length?'':'<p class="muted">Сначала добавьте на доске поле: текст, число, вариант, дату или отметку.</p>'}</details>`;
}
...
export async function openPageRecordAction(context,block,actionId,recordId,deps){
 ...
 async function load(){const turn=++serial;preview=null;apply.disabled=true;reload.disabled=true;status.textContent='Проверяем актуальную запись…';try{
  ...
  if(turn!==serial||!active()||!review.isConnected)return;
  preview=value;review.innerHTML=`<h3>${e(value.label)}</h3><p>${e(value.title)}</p>${value.sourceField?`<p class="muted">Источник: ${e(value.sourceField.name)} · ${e(displayField(value.sourceField,value.sourceValue)||'Не указано')}</p>`:''}${value.condition?`<p class="muted">${e(conditionDescription(value.condition,{fields:[value.conditionField]},displayField))}</p>`:''}<strong>${e(value.field.name)}</strong><div class="app-action-comparison"><div><small>Сейчас</small><p>${e(displayField(value.field,value.before)||'Не указано')}</p></div><div><small>После подтверждения</small><p>${e(displayField(value.field,value.after)||'Не указано')}</p></div></div>`;status.textContent=value.allowed===false?value.conditionReason:'Изменение ещё не сохранено.';apply.disabled=value.allowed===false;

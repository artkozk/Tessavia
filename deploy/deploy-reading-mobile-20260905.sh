#!/usr/bin/env bash
# Application-only release for the compact Homegroup reading screen.
...
[[ "$RELEASE_NAME" =~ ^20260905-reading-mobile-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-reading-mobile
release="$root/releases/$RELEASE_NAME"
previous=$(readlink -f "$root/current")
backup="/var/lib/business-control/backups/pre-reading-mobile-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ "$result" -eq 0 && -n "$dry" && "$dry" == /tmp/tessavie-reading-mobile-check.* ]]; then
    rm -rf -- "$dry"
  elif [[ -n "$dry" ]]; then
...
dry=$(mktemp -d /tmp/tessavie-reading-mobile-check.XXXXXX)
...
check_http() {
  ...
  grep -qF 'data-reading-log-current' "$dry/reading.js"
  grep -qF 'class="reading-book-picker"' "$dry/reading.js"
  grep -qF 'Открыть следующую главу' "$dry/reading.js"
  grep -qF 'Добавить пометку' "$dry/reading.js"
  curl --max-time 15 -fsS "$base/styles.css?v=20260905-reading-mobile-2" > "$dry/styles.css"
  grep -qF '.reading-page > .reading-heading { display: none; }' "$dry/styles.css"
  grep -qF '.reading-catalog { display: none; }' "$dry/styles.css"
  grep -qF '.reading-testament-book[hidden] { display: none; }' "$dry/styles.css"
  curl --max-time 15 -fsS "$base/sw.js" > "$dry/sw.js"
  grep -qF 'tessavie-shell-20260905-reading-mobile-2' "$dry/sw.js"

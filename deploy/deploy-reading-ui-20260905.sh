...
[[ "$RELEASE_NAME" =~ ^20260905-reading-ui-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-reading-ui
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-reading-ui-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-reading-ui-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-reading-ui-check.XXXXXX)
...
check_http() {
  ...
  grep -qF 'waitingPingPreview' "$dry/personal-waiting.js"
  ...
  test "$(curl --max-time 10 -s -X POST -H 'Content-Type: application/json' -d '{}' -o /dev/null -w '%{http_code}' "$base/api/personal/waiting/not-owned/ping")" = 401
  ...
  curl --max-time 15 -fsS "$base/reminder-settings.js?v=20260904-reminder-digests-1" > "$dry/reminder-settings.js"
  grep -qF 'habitsEnabled' "$dry/reminder-settings.js"
  grep -qF 'dailyDigestEnabled' "$dry/reminder-settings.js"
  grep -qF 'weeklyDigestWeekday' "$dry/reminder-settings.js"
  ...
  curl --max-time 15 -fsS "$base/reading.js?v=20260905-reading-ui-4" > "$dry/reading.js"
  grep -qF 'createReadingUI' "$dry/reading.js"
  grep -qF 'reading-person-avatar' "$dry/reading.js"
  grep -qF 'reading-journal-day' "$dry/reading.js"
  grep -qF 'rank.displayName' "$dry/reading.js"
  grep -qF 'role="tablist"' "$dry/reading.js"
  grep -qF 'currentTab.offsetLeft - (tabStrip.clientWidth - currentTab.offsetWidth) / 2' "$dry/reading.js"
  ...
  curl --max-time 15 -fsS "$base/personal-today.js?v=20260905-reading-ui-4" > "$dry/personal-today.js"
  grep -qF 'createPersonalTodayUI' "$dry/personal-today.js"
  grep -qF 'Моя работа в проектах' "$dry/personal-today.js"
  grep -qF 'Требует внимания' "$dry/personal-today.js"
  ...
  grep -q '20260905-reading-ui-4' "$dry/index.html"
  ...
  grep -qF '.reading-person-avatar' "$dry/styles.css"
  grep -qF '.reading-journal-day' "$dry/styles.css"
  grep -qF '@media (max-width: 380px)' "$dry/styles.css"
  ...
  grep -qF "item.entityType==='personal_digest'" "$dry/app.js"
  ! grep -qF 'Администратор управляет только этой командой и не получает доступ к другим стартапам.' "$dry/app.js"
  ...
  grep -qF "20260905-reading-ui-4" "$dry/sw.js"
...
test "$(sqlite3 "$db" "SELECT COUNT(*) FROM schema_migrations WHERE version='053_personal_waiting_pings.sql';")" = 1
test "$(sqlite3 "$db" "SELECT COUNT(*) FROM schema_migrations WHERE version='054_reminder_digests.sql';")" = 1
test "$(sqlite3 "$db" "SELECT COUNT(*) FROM schema_migrations WHERE version='055_reading_reflections.sql';")" = 1
test "$(sqlite3 "$db" "SELECT COUNT(*) FROM reminder_preferences WHERE daily_digest_enabled<>0 OR weekly_digest_enabled<>0;")" = 0

...
func RunReminderWorker(ctx context.Context, store *Store) {
	...
	for {
		...
		cancel()
		tick, cancel = context.WithTimeout(ctx, 10*time.Second)
		if err := deliverHabitReminders(tick, store, time.Now()); err != nil && ctx.Err() == nil {
			log.Printf("habit reminder tick: %v", err)
		}
		cancel()
		tick, cancel = context.WithTimeout(ctx, 10*time.Second)
		if err := deliverReminderDigests(tick, store, time.Now()); err != nil && ctx.Err() == nil {
			log.Printf("reminder digest tick: %v", err)
		}
		cancel()
		select {
		case <-ctx.Done():
...
const notificationFresh = `(` + digestReminderFresh + ` AND ` + habitReminderFresh + ` AND ` + personalReminderFresh + ` AND (n.type<>'deadline' OR EXISTS (

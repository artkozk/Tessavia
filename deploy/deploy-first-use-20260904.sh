...
[[ "$RELEASE_NAME" =~ ^20260904-first-use-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-first-use
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-first-use-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-first-use-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-first-use-check.XXXXXX)
...
python3 /tmp/verify-journey-migration.py "$backup/business-control.db" "$dry/check.db"
...
check_http() {
  ...
  grep -q '20260904-first-use-4' "$dry/index.html"
  ...
  grep -qF "20260904-first-use-4" "$dry/sw.js"

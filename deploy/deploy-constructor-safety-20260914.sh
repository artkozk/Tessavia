...
[[ "$RELEASE_NAME" =~ ^20260914-constructor-safety-[a-f0-9]+$ ]]
[[ "$CANDIDATE_COMMIT" =~ ^[a-f0-9]{40}$ ]]
release_suffix=${RELEASE_NAME#20260914-constructor-safety-}
[[ "$CANDIDATE_COMMIT" == "$release_suffix"* ]]
[[ "$EXPECTED_SHA256" =~ ^[a-f0-9]{64}$ ]]
test "$EXPECTED_PREVIOUS" = /opt/business-control/releases/20260914-field-conversion-ae9402f

exec 9>/run/business-control-deploy.lock
flock -n 9
root=/opt/business-control
release="$root/releases/$RELEASE_NAME"
staged=/tmp/business-control-constructor-safety
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-constructor-safety-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
release_created=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-constructor-safety-check.* ]]; then rm -rf -- "$dry"; fi
  if [[ "$result" -ne 0 && "$switch_started" -eq 1 ]]; then
    ...
    # This release changes editor safety without a schema change. Validate
    ...
    if python3 /tmp/verify-constructor-safety-database.py --rollback-safe "$db"; then
...
printf '%s  %s\n' '088d817fca25ec44bfa583c694b72600feec625ab5da6fa4fb0d3d672fce3e16' "$previous/business-control" | sha256sum -c -
test ! -e "$release"
printf '%s  %s\n' "$EXPECTED_SHA256" "$staged" | sha256sum -c -
python3 - <<'PY'
import socket
with socket.socket() as sock:
    sock.bind(('127.0.0.1', 18640))
PY
...
dry=$(mktemp -d /tmp/tessavie-constructor-safety-check.XXXXXX)
cp "$backup/business-control.db" "$dry/check.db"
mkdir "$dry/uploads"
chown -R business-control:business-control "$dry"
runuser -u business-control -- env BUSINESS_REMINDERS_ENABLED=false BUSINESS_ADDRESS=127.0.0.1:18640 BUSINESS_DATABASE_PATH="$dry/check.db" BUSINESS_UPLOAD_PATH="$dry/uploads" "$release/business-control" > "$dry/server.log" 2>&1 &
dry_pid=$!
for attempt in $(seq 1 30); do
  if curl --max-time 3 -fsS http://127.0.0.1:18640/api/health > "$dry/health.json" 2>/dev/null; then break; fi
  test "$attempt" -lt 30
  sleep 1
done
python3 /tmp/verify-constructor-safety-database.py "$backup/business-control.db" "$dry/check.db"

check_http() {
  local base="$1"
  curl --max-time 15 -fsS "$base/" > "$dry/index.html"
  grep -qF 'app.js?v=20260914-constructor-safety-1' "$dry/index.html"
  grep -qF 'styles.css?v=20260914-constructor-safety-1' "$dry/index.html"
  grep -qF 'settings-hub.css?v=20260914-constructor-safety-1' "$dry/index.html"
  grep -qF 'personal-finance.css?v=20260914-constructor-safety-1' "$dry/index.html"
  grep -qF 'page-sheets.css?v=20260914-constructor-safety-1' "$dry/index.html"
  grep -qF 'page-components.css?v=20260914-constructor-safety-1' "$dry/index.html"
  curl --max-time 15 -fsS "$base/app.js?v=20260914-constructor-safety-1" > "$dry/app.js"
  grep -qF 'chat-workspace.js?v=20260914-constructor-safety-1' "$dry/app.js"
  ...
  grep -qF 'page-apps.js?v=20260914-constructor-safety-1' "$dry/app.js"
  grep -qF 'settings-hub.js?v=20260914-constructor-safety-1' "$dry/app.js"
  ...
  curl --max-time 15 -fsS "$base/habit-tracker.js?v=20260914-constructor-safety-1" > "$dry/habit-tracker.js"
  ...
  curl --max-time 15 -fsS "$base/settings-hub.js?v=20260914-constructor-safety-1" > "$dry/settings-hub.js"
  grep -qF 'createSettingsHub' "$dry/settings-hub.js"
  grep -qF 'settings-hub' "$dry/settings-hub.js"
  curl --max-time 15 -fsS "$base/settings-hub.css?v=20260914-constructor-safety-1" > "$dry/settings-hub.css"
  grep -qF '.settings-hub' "$dry/settings-hub.css"
  grep -qF 'personal-finance.js?v=20260914-constructor-safety-1' "$dry/app.js"
  curl --max-time 15 -fsS "$base/personal-finance.js?v=20260914-constructor-safety-1" > "$dry/personal-finance.js"
  grep -qF 'createPersonalFinanceUI' "$dry/personal-finance.js"
  grep -qF 'data-personal-finance' "$dry/personal-finance.js"
  curl --max-time 15 -fsS "$base/personal-finance.css?v=20260914-constructor-safety-1" > "$dry/personal-finance.css"
  ...
  grep -qF 'field-conversion.js?v=20260914-constructor-safety-1' "$dry/app.js"
  curl --max-time 15 -fsS "$base/field-conversion.js?v=20260914-constructor-safety-1" > "$dry/field-conversion.js"
  ...
  curl --max-time 15 -fsS "$base/personal-navigation.js?v=20260914-constructor-safety-1" > "$dry/personal-navigation.js"
  ...
  curl --max-time 15 -fsS "$base/page-apps.js?v=20260914-constructor-safety-1" > "$dry/page-apps.js"
  curl --max-time 15 -fsS "$base/page-record-bindings.js?v=20260914-constructor-safety-1" > "$dry/bindings.js"
  grep -qF 'recordBindingText' "$dry/bindings.js"
  grep -qF 'data-binding-remove' "$dry/bindings.js"
  curl --max-time 15 -fsS "$base/page-element-styles.js?v=20260914-constructor-safety-1" > "$dry/elements.js"
  grep -qF 'applyElementStyles' "$dry/elements.js"
  grep -qF 'data-element-reset' "$dry/elements.js"
  curl --max-time 15 -fsS "$base/page-form-elements.js?v=20260914-constructor-safety-1" > "$dry/form-elements.js"
  ...
  grep -qF 'page-sheets.js?v=20260914-constructor-safety-1' "$dry/page-apps.js"
  grep -qF 'createPageSheetUI' "$dry/page-apps.js"
  curl --max-time 15 -fsS "$base/page-sheets.js?v=20260914-constructor-safety-1" > "$dry/page-sheets.js"
  ...
  curl --max-time 15 -fsS "$base/page-sheets.css?v=20260914-constructor-safety-1" > "$dry/page-sheets.css"
  ...
  curl --max-time 15 -fsS "$base/page-labels.js?v=20260914-constructor-safety-1" > "$dry/page-labels.js"
  grep -qF 'updatePageTexts' "$dry/page-labels.js"
  curl --max-time 15 -fsS "$base/page-components.js?v=20260914-constructor-safety-1" > "$dry/page-components.js"
  grep -qF '/api/page-app/components' "$dry/page-components.js"
  grep -qF 'normalizeComponentDrafts' "$dry/page-components.js"
  grep -qF 'page-block-kind.js?v=20260914-constructor-safety-1' "$dry/page-apps.js"
  curl --max-time 15 -fsS "$base/page-block-kind.js?v=20260914-constructor-safety-1" > "$dry/page-block-kind.js"
  grep -qF 'createBlockKindGuard' "$dry/page-block-kind.js"
  curl --max-time 15 -fsS "$base/page-components.css?v=20260914-constructor-safety-1" > "$dry/page-components.css"
  ...
  curl --max-time 15 -fsS "$base/page-block-visibility.js?v=20260914-constructor-safety-1" > "$dry/visibility.js"
  ...
  curl --max-time 15 -fsS "$base/page-forms.js?v=20260914-constructor-safety-1" > "$dry/page-forms.js"
  ...
  curl --max-time 15 -fsS "$base/page-record-actions.js?v=20260914-constructor-safety-1" > "$dry/page-record-actions.js"
  ...
  curl --max-time 15 -fsS "$base/page-action-conditions.js?v=20260914-constructor-safety-1" > "$dry/page-action-conditions.js"
  ...
  curl --max-time 15 -fsS "$base/field-conflicts.js?v=20260914-constructor-safety-1" > "$dry/field-conflicts.js"
  grep -qF 'reviewFieldConflict' "$dry/field-conflicts.js"
  grep -qF 'fieldEditBaseline' "$dry/app.js"
  curl --max-time 15 -fsS "$base/page-data-sources.js?v=20260914-constructor-safety-1" > "$dry/data.js"
...
PYBOOK
  ...
  curl --max-time 15 -fsS "$base/chat-workspace.js?v=20260914-constructor-safety-1" > "$dry/chat.js"
  grep -qF 'createChatWorkspaceUI' "$dry/chat.js"
  curl --max-time 15 -fsS "$base/life-map.js?v=20260914-constructor-safety-1" > "$dry/life.js"
  grep -qF 'life-map-overview' "$dry/life.js"
  curl --max-time 15 -fsS "$base/chat-groups.js?v=20260914-constructor-safety-1" > "$dry/groups.js"
  grep -qF 'createChatGroupUI' "$dry/groups.js"
  curl --max-time 15 -fsS "$base/personal-calendar.js?v=20260914-constructor-safety-1" > "$dry/calendar.js"
  grep -qF 'openRecurrence' "$dry/calendar.js"
  grep -qF 'bindPersonalRecurrenceScope' "$dry/app.js"
  curl --max-time 15 -fsS "$base/personal-today.js?v=20260914-constructor-safety-1" > "$dry/today.js"
  ...
  curl --max-time 15 -fsS "$base/styles.css?v=20260914-constructor-safety-1" > "$dry/styles.css"
  grep -qF -- '--chat-menu-left' "$dry/styles.css"
  curl --max-time 15 -fsS "$base/page-composition.js?v=20260914-constructor-safety-1" > "$dry/composition.js"
  grep -qF 'duplicateBlockTree' "$dry/composition.js"
  grep -qF 'groupChoices' "$dry/composition.js"
  curl --max-time 15 -fsS "$base/page-calculations.js?v=20260914-constructor-safety-1" > "$dry/calculations.js"
  grep -qF 'evaluateCalculation' "$dry/calculations.js"
  grep -qF 'bindCalculationConfig' "$dry/calculations.js"
  curl --max-time 15 -fsS "$base/page-record-card.js?v=20260914-constructor-safety-1" > "$dry/card.js"
  grep -qF "recordCardBody" "$dry/card.js"
  grep -qF "bindRecordCardConfig" "$dry/card.js"
  curl --max-time 15 -fsS "$base/sw.js" > "$dry/sw.js"
  grep -qF 'tessavie-shell-20260914-constructor-safety-1' "$dry/sw.js"
  grep -qF 'settings-hub.js?v=20260914-constructor-safety-1' "$dry/sw.js"
  grep -qF 'settings-hub.css?v=20260914-constructor-safety-1' "$dry/sw.js"
  grep -qF 'personal-finance.js?v=20260914-constructor-safety-1' "$dry/sw.js"
  grep -qF 'personal-finance.css?v=20260914-constructor-safety-1' "$dry/sw.js"
  grep -qF 'page-sheets.js?v=20260914-constructor-safety-1' "$dry/sw.js"
  grep -qF 'page-sheets.css?v=20260914-constructor-safety-1' "$dry/sw.js"
...
check_http http://127.0.0.1:18640
kill "$dry_pid"
wait "$dry_pid" 2>/dev/null || true
dry_pid=''
printf 'DRY_RUN=ok\n'

# Baseline supports the same data contract, including populated expense/library
# tables. Trial its startup on the candidate-checked copy; require every value
# and migration receipt to remain unchanged before touching production.
python3 /tmp/verify-constructor-safety-database.py --rollback-safe "$dry/check.db"
runuser -u business-control -- env BUSINESS_REMINDERS_ENABLED=false BUSINESS_ADDRESS=127.0.0.1:18640 BUSINESS_DATABASE_PATH="$dry/check.db" BUSINESS_UPLOAD_PATH="$dry/uploads" "$previous/business-control" > "$dry/rollback-server.log" 2>&1 &
dry_pid=$!
for attempt in $(seq 1 30); do
  if curl --max-time 3 -fsS http://127.0.0.1:18640/api/health > "$dry/rollback-health.json" 2>/dev/null; then break; fi
  test "$attempt" -lt 30
  sleep 1
done
curl --max-time 15 -fsS http://127.0.0.1:18640/ > "$dry/rollback-index.html"
grep -qF 'app.js?v=20260914-field-conversion-1' "$dry/rollback-index.html"
python3 /tmp/verify-constructor-safety-database.py "$backup/business-control.db" "$dry/check.db"
kill "$dry_pid"
wait "$dry_pid" 2>/dev/null || true
dry_pid=''
...
python3 /tmp/verify-constructor-safety-database.py --rollback-safe "$backup/cutover.db"
...
python3 /tmp/verify-constructor-safety-database.py --rollback-safe "$db"

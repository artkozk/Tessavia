...
function harness() {
  ...
  const context = vm.createContext({ state, calendarPresentation:()=>({includeWork:!!state.includeWork}),personalCalendarUI:{recurrences:()=>state.recurrences||[],entries:()=>state.workEntries||[]},localDateKey: date => date.toISOString().slice(0,10), isActiveRecord: item => item.status !== 'completed', isWorkRecord: () => true });
...
test('future repetitions respect status and space filters independently of work visibility',()=>{
 const h=harness();h.state.recurrences=[{id:'forecast',calendarKind:'recurrence',status:'planned',occurrenceDate:'2026-09-14'}];
 assert.equal(h.run('plannerItems().length'),1);assert.equal(h.run("plannerMatchesDay(plannerItems()[0],'2026-09-14',true)"),true);
 h.state.calendarStatus='done';assert.equal(h.run('plannerItems().length'),0);
 h.state.calendarStatus='active';h.state.calendarScope='project';assert.equal(h.run('plannerItems().length'),0);
 assert.equal(h.state.personal.plans.length,0);
});

...
: "${ASSET_VERSION:=20260903-performance-1}"
: "${BRAND_VERSION:=linked-1}"
: "${EXPECTED_LOGO_WIDTH:=128px}"
[[ "$ASSET_VERSION" = 20260903-performance-1 ]]
[[ "$BRAND_VERSION" =~ ^(supplied|linked)-1$ ]]
[[ -z "$EXPECTED_LOGO_WIDTH" || "$EXPECTED_LOGO_WIDTH" =~ ^[0-9]{2,3}px$ ]]
[[ "$RELEASE_NAME" =~ ^20260903-performance-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-performance
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-performance-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''; dry_pid=''; switch_started=0; nginx_modified=0; snippet_existed=0
nginx_config=$(readlink -f /etc/nginx/sites-enabled/business-control)
[[ "$nginx_config" = /etc/nginx/sites-available/business-control ]]
snippet=/etc/nginx/snippets/tessavie-performance.conf
cleanup() {
  result=$?
  trap - EXIT
  if [[ -n "$dry_pid" ]]; then kill "$dry_pid" 2>/dev/null || true; wait "$dry_pid" 2>/dev/null || true; fi
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-performance-check.* ]]; then rm -rf -- "$dry"; fi
  if [[ "$result" -ne 0 && "$nginx_modified" -eq 1 ]]; then
    cp "$backup/nginx-business-control.conf" "$nginx_config"
    if [[ "$snippet_existed" -eq 1 ]]; then cp "$backup/nginx-performance.conf" "$snippet"; else rm -f -- "$snippet"; fi
...
python3 - <<'PY'
import socket
with socket.socket() as s: s.bind(('127.0.0.1', 18553))
PY
...
dry=$(mktemp -d /tmp/tessavie-performance-check.XXXXXX)
cp "$backup/business-control.db" "$dry/check.db"
mkdir "$dry/uploads"
chown -R business-control:business-control "$dry"
runuser -u business-control -- env BUSINESS_ADDRESS=127.0.0.1:18553 BUSINESS_DATABASE_PATH="$dry/check.db" BUSINESS_UPLOAD_PATH="$dry/uploads" "$release/business-control" > "$dry/server.log" 2>&1 &
dry_pid=$!
for attempt in $(seq 1 30); do
  if curl --max-time 3 -fsS http://127.0.0.1:18553/api/health > "$dry/health.json" 2>/dev/null; then break; fi
...
check_http() {
  ...
  grep -qF 'const pendingAPIReads = new Map();' "$dry/app.js"
  grep -qF 'async function refreshProjectCompanions(workspace)' "$dry/app.js"
  grep -qF 'function openPersonalCapture()' "$dry/app.js"
...
check_http http://127.0.0.1:18553
kill "$dry_pid"; wait "$dry_pid" 2>/dev/null || true; dry_pid=''
printf 'DRY_RUN=ok\n'
cp "$nginx_config" "$backup/nginx-business-control.conf"
if [[ -e "$snippet" ]]; then cp "$snippet" "$backup/nginx-performance.conf"; snippet_existed=1; fi
nginx_modified=1
install -m 0644 /tmp/tessavie-performance.nginx.conf "$snippet"
python3 - "$nginx_config" <<'PYCONFIG'
from pathlib import Path
import sys
p=Path(sys.argv[1]); text=p.read_text()
include='    include /etc/nginx/snippets/tessavie-performance.conf;'
assert 'proxy_pass http://127.0.0.1:8522;' in text
assert 'listen 443 ssl;' in text
needle='    server_name control.e-rd.ru;'
assert needle in text
if include not in text: text=text.replace(needle,needle+'\n'+include,1)
p.write_text(text)
PYCONFIG
nginx -t
...
curl --max-time 15 -fsS -H 'Accept-Encoding: gzip' -D "$backup/gzip-headers.txt" https://control.e-rd.ru/app.js > "$backup/app.js.gz"
grep -qi 'content-encoding: gzip' "$backup/gzip-headers.txt"
python3 - "$backup/app.js.gz" "$dry/app.js" <<'PYGZIP'
import gzip,sys
compressed=open(sys.argv[1],'rb').read()
original=open(sys.argv[2],'rb').read()
assert gzip.decompress(compressed)==original
print('JS_TRANSFER_BYTES='+str(len(compressed))+' ORIGINAL='+str(len(original)))
PYGZIP
curl --http2 --max-time 15 -fsS -o /dev/null -w '%{http_version}' https://control.e-rd.ru/api/health > "$backup/http-version.txt"
test "$(cat "$backup/http-version.txt")" = 2
curl --max-time 15 -fsS -D "$backup/cache-headers.txt" -o /dev/null "https://control.e-rd.ru/app.js?v=$ASSET_VERSION"
grep -qi 'Cache-Control: public, max-age=31536000, immutable' "$backup/cache-headers.txt"
printf 'TRANSPORT=ok\n'

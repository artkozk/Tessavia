"""Record exactly the two finance groups/constructor tasks after a verified release.
...
identity, running-binary hash, backups, public hooks and schema075 are checked afresh.
"""
import argparse
import datetime
import hashlib
...
TASKS = (
    {'id': 'ad787975b390e40313472673274982d4',
     'title': 'P1 · Группы финансовых операций и связи с карточками и целями',
     'request': '[request:finance-groups-relations-20260915]',
     'result': '[release:finance-groups-relations-20260915]'},
    {'id': '16fbc52ae06d4d296dff09e93f3319ba',
     'title': 'P1 · Настоящий блок финансов в конструкторе для каждого пространства',
     'request': '[request:finance-builder-block-20260915]',
     'result': '[release:finance-builder-block-20260915]'},
)
TASK_IDS = frozenset(task['id'] for task in TASKS)
CACHE = '20260915-finance-constructor-1'
PREVIOUS = '/opt/business-control/releases/20260915-team-finance-media-0a7a6eb'
PREVIOUS_COMMIT = '0a7a6ebcfd4c9ada3153c94a5212c26ee0249e5f'
PREVIOUS_SHA256 = '47d9015b40a1ed95615ace3c7e4bcd88a797d85d2623503770e92bd653774317'
WORKSPACE = 'bizflow-team'
OPERATOR = 1
SCENARIOS = (
    'groupsCreateAssignFilterTotals', 'groupArchiveRestore', 'linksCreateReadRevoke',
    'personalTeamIsolation', 'linkedBudgetReadOnly', 'constructorAddSaveReload',
    'constructorMultipleBlocksAndActions', 'portableKitWithoutFinancialData',
    'staleContextAndDraftRecovery',
)
...
def validate_acceptance(value):
    ...
    for name, minimum in (('goTestCount', 435), ('nodeTests', 664), ('buildInputs', 524), ('verifierTests', 13)):
...
def validate_identity(release, commit, sha256, backup):
    ...
    match = re.fullmatch(r'/opt/business-control/releases/20260915-finance-constructor-([a-f0-9]{7,40})', release)
    require(match and commit.startswith(match.group(1)), 'Release does not identify this finance-constructor commit')
    require(re.fullmatch(r'/var/lib/business-control/backups/pre-finance-constructor-[0-9]{8}T[0-9]{6}Z', str(backup)), 'Unexpected same-host backup path')
...
def verify_release(opener, db, release, commit, sha256, backup):
    ...
    for base in ('http://127.0.0.1:8522', 'https://control.e-rd.ru'):
        ...
        for asset in ('app.js', 'styles.css', 'page-finance.css'):
            require((asset + '?v=' + CACHE).encode() in index, 'Index cache identity differs')
        for name, markers in {
            'app.js': ('const teamFinanceUI', 'renderTeamFinance', 'onFinanceAction'),
            'personal-finance.js': ('X-Finance-Source', 'X-Finance-Revision', 'financeOrganizationPayload', 'data-finance-history-category', 'data-finance-related'),
            'settings-hub.js': ('team-finance',),
            'page-apps.js': ('createPageFinanceUI', 'pageFinanceConfigMarkup', 'financeBlocks.mount'),
            'page-finance.js': ('validatePageFinanceOverview', 'sourceRevision', 'pageFinanceOverviewMarkup'),
            'page-finance.css': ('.page-finance-accounts', '.page-finance-actions'),
        }.items():
            source = read_bytes(opener, base + '/' + name + '?v=' + CACHE)
            require(all(marker.encode() in source for marker in markers), 'Missing published finance constructor implementation: ' + name)
        worker = read_bytes(opener, base + '/sw.js')
        require(('tessavie-shell-' + CACHE).encode() in worker, 'Service worker cache differs')
        for name in ('select-positioning.js', 'page-media.js', 'page-media.css', 'page-finance.js', 'page-finance.css'):
    ...
    spec = importlib.util.spec_from_file_location('finance_constructor_exact_schema', ROOT / 'verify-finance-constructor-database.py')
    verifier = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(verifier)
    require(verifier.validate(db) == 75, 'Expected exact deployed schema 075 and 143 tables')
...
def evidence_for(release, commit, sha256, backup, acceptance):
    ...
    shared = (f'Проверенный source {commit}; release {release}; SHA256 {sha256}; '
              ...
              'Миграция 075 добавила четыре таблицы; прежние 139 таблиц и их строки сохранены; '
              'после выпуска подтверждена точная схема 075/143 таблиц. Совместимость прежнего '
    ...
    contents = (
        'Личный и командный учёт поддерживают свои группы операций, назначение '
        'доходам и расходам, фильтр и суммы по группам. Группы не заменяют счета; '
        'архивирование обратимо и сохраняет исторические назначения. '
        'Необязательные связи с доступными карточками, целями и личными делами '
        'создаются из формы операции, права проверяются при сохранении и чтении. '
        'Недоступные названия не раскрываются. Повторы, ревизии, черновики, '
        'изоляция пространств и чтение подключённого бюджета проверены. '
        'Изменение группы или связи не переписывает арифметику дохода/расхода. '
        'Настройки групп остаются в общем центре настроек.',
        'Любой пользователь собирает блок «Финансы» через единый конструктор. '
        'Он показывает учёт пространства установленной страницы: личный либо '
        'командный, включая явно подписанный связанный источник только для чтения. '
        'Остаток, счета, последние операции, быстрые действия, подписи и '
        'оформление настраиваются и сохраняются. Несколько блоков обновляются '
        'после записи через общий контроллер, старый контекст не меняет источник. '
        'Набор переносит структуру и оформление без частных счетов, операций '
        'и прав автора. Публичный предпросмотр не показывает чужой бюджет. '
        'Документ: docs/architecture/FINANCE_CONSTRUCTOR_BLOCK_2026_09_15.md.',
    )
...
def main():
    ...
    parser.add_argument('--receipt', type=Path, default=Path('/tmp/tessavie-finance-constructor-results-20260915.json'))
    ...
    import fcntl
    lock = open('/run/business-control-finance-constructor-results.lock', 'a')
    ...
    try:
        ...
        receipt.update({'checkedAt': helpers.stamp(now), 'release': args.release, 'sourceCommit': args.commit,
                        ...
                        'previousBinarySHA256': PREVIOUS_SHA256, 'schemaVersion': 75, 'tableCount': 143})
    finally:

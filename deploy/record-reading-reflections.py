"""Verify the production reading journal/grid and record the scoped platform task."""
...
title = "Упростить чтение Домашки и добавить отдельные мысли"
marker = "[verified:reading-grid-" + args.commit[:7] + "]"
try:
    ...
    if matches:
        ...
        task = api(
            ...
            "POST",
            {
                ...
                "title": title,
                "description": (
                    "Открывать выбранную книгу сразу с круглой сеткой глав, отмечать одну "
                    "полную личную главу одним нажатием и разделить компактный каталог на "
                    "Ветхий и Новый Завет. Дополнительно разрешить сохранить самостоятельную "
                    "мысль без фиктивной отметки чтения, позже редактировать и явно публиковать "
                    "только текущей домашней группе."
                ),
                "status": "in_progress",
                "ownerId": user[0],
                "priority": "normal",
    ...
    proof = "\n".join(
        [
            marker,
            "Выпущены открытая книга, пять мобильных колонок круглых глав, однокнопочная приватная отметка и две таблицы книг по Заветам. Повтор сегодняшней главы открывает пометку; диапазон и частичное чтение сохранены отдельным действием.",
    ...
    if detail["record"]["status"] != "completed":
        api(
            ...
            "POST",
            {
                "result": "Чтение открывается выбранной книгой и отмечает главу одним нажатием; книги разделены по Заветам. Дневник принимает отдельную приватную мысль без отметки и позволяет позже её отредактировать или явно показать группе.",

...
export function blockVisible(block,definition,marks={}){
 ...
 const c=block.visibility;if(!c)return true;if(c.mode||c.conditions){if(!['all','any'].includes(c.mode)||!c.conditions?.length||c.conditions.length>8||c.conditions.some(x=>x.mode||x.conditions))return false;const results=c.conditions.map(leaf=>blockVisible({...block,visibility:leaf},definition,marks));return c.mode==='all'?results.every(Boolean):results.some(Boolean);}
...
export function bindVisibilityConfig(root,block,definition,persist,draw){
 root.querySelector('[data-visibility-add]')?.addEventListener('click',()=>{const leaves=visibilityLeaves(block.visibility);if(!leaves.length||leaves.length>=8)return;const sources=definition.blocks.filter(b=>b.kind==='tracker'&&b.id!==block.id);const source=sources.find(b=>!leaves.some(c=>c.source===b.id))||sources[0];if(!source)return;block.visibility={mode:block.visibility.mode||'all',conditions:[...leaves,{source:source.id,metric:'remaining',operator:'eq',value:0}]};persist();draw();});

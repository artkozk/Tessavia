...
[[ "$RELEASE_NAME" =~ ^20260903-navigation-context-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-navigation-context
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-navigation-context-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''; dry_pid=''; switch_started=0
cleanup() {
  result=$?
  trap - EXIT
  if [[ -n "$dry_pid" ]]; then kill "$dry_pid" 2>/dev/null || true; wait "$dry_pid" 2>/dev/null || true; fi
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-navigation-context-check.* ]]; then rm -rf -- "$dry"; fi
...
check_http() {
  ...
  grep -q '20260903-navigation-context-1' "$dry/index.html"

"""Record exactly the two media versions/select delivery tasks after a verified release.
...
identity, running-binary hash, backups, public hooks and schema076 are checked afresh.
"""
import argparse
import datetime
import hashlib
...
CACHE = '20260915-media-variants-1'
PREVIOUS = '/opt/business-control/releases/20260915-finance-constructor-e5ecd4b'
PREVIOUS_COMMIT = 'e5ecd4bdea3057d56bdf7134e35ee1a436de480d'
PREVIOUS_SHA256 = '1790be5b27e3702e945a762f859545bfb2d5232576e14b3852f009e88506160d'
...
SCENARIOS = (
    ...
    'authorizationIsolation', 'selectAnchorScrollAndLayout', 'selectFinanceDraftPreserved',
    'updateForegroundAndSettings', 'updateUnsavedWorkGuard',
)
...
def validate_acceptance(value):
    ...
    for name, minimum in (('goTestCount', 442), ('nodeTests', 670), ('buildInputs', 524), ('verifierTests', 21)):
...
def validate_identity(release, commit, sha256, backup):
    ...
    match = re.fullmatch(r'/opt/business-control/releases/20260915-media-variants-([a-f0-9]{7,40})', release)
    require(match and commit.startswith(match.group(1)), 'Release does not identify this media-variants commit')
    require(re.fullmatch(r'/var/lib/business-control/backups/pre-media-variants-[0-9]{8}T[0-9]{6}Z', str(backup)), 'Unexpected same-host backup path')
...
def verify_release(opener, db, release, commit, sha256, backup):
    ...
    for base in ('http://127.0.0.1:8522', 'https://control.e-rd.ru'):
        ...
        for name, markers in {
            'app.js': ('const teamFinanceUI', 'renderTeamFinance', 'onFinanceAction', 'createMediaVariantsUI', 'reloadUpdatedInterface', 'shellUpdatePending'),
            ...
            require(all(marker.encode() in source for marker in markers), 'Missing published media/select-delivery implementation: ' + name)
    ...
    spec = importlib.util.spec_from_file_location('media_variants_exact_schema', ROOT / 'verify-media-variants-database.py')
    verifier = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(verifier)
    require(verifier.validate(db) == 76, 'Expected exact deployed schema 076 and 148 tables')
...
def evidence_for(release, commit, sha256, backup, acceptance):
    ...
    shared = (f'Проверенный source {commit}; release {release}; SHA256 {sha256}; '
              ...
              'Миграция 076 добавила пять таблиц; прежние 143 таблицы и их строки сохранены; '
              'после выпуска подтверждена точная схема 076/148 таблиц. Совместимость прежнего '
    ...
    contents = (
        'В карточках, сохранённых личных заметках и медиаблоках доступны именованные '
        'варианты материалов. Новая версия добавляется с пометкой, прежний файл '
        'сохраняется; история показывает автора и дату, выбор версии явный, архив '
        'обратимый. Два изображения можно сравнить. Старые вложения доступны без '
        'переписывания и массового backfill. Загрузка сначала подтверждает файл, '
        'затем метаданные варианта; повтор использует сохранённый ключ. Старые '
        'локальные очереди заметок и страниц не теряют Blob или подтверждение. '
        'Проверены права, изоляция, смена контекста, сохранность текста заметки и '
        'отсутствие частных ID в переносимом наборе. '
        'Документы: docs/architecture/MEDIA_VARIANTS_UI_2026_09_15.md и '
        'docs/architecture/NOTE_PAGE_MEDIA_VARIANTS_ADAPTERS_2026_09_15.md.',
        'Повторно проверены списки источника и отправителя в финансах: положение '
        'около поля, ограничение видимой областью, прокрутка и изменение раскладки. '
        'Форма сохраняет введённую сумму и пометку. Прежние снимки от 14 сентября '
        'предшествуют выпуску позиционирования 15 сентября; сами по себе они не '
        'доказывают дефект новой геометрии. Устранён подтверждённый недостаток '
        'доставки обновления: доступная версия остаётся в общем центре настроек '
        'и снова предлагается при возвращении во вкладку. Автоматическая '
        'перезагрузка не уничтожает несохранённую работу; обновление выполняется '
        'через общий контролируемый переход. Физический мобильный браузер не '
        'выдаётся за проверенный по одной эмуляции ширины.',
    )
...
def main():
    ...
    parser.add_argument('--receipt', type=Path, default=Path('/tmp/tessavie-media-variants-results-20260915.json'))
    ...
    import fcntl
    lock = open('/run/business-control-media-variants-results.lock', 'a')
    ...
    try:
        ...
        receipt.update({'checkedAt': helpers.stamp(now), 'release': args.release, 'sourceCommit': args.commit,
                        ...
                        'previousBinarySHA256': PREVIOUS_SHA256, 'schemaVersion': 76, 'tableCount': 148})
    finally:

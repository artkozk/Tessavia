...
export function settingsHubSections(context = {}) {
  ...
  const groups = {
    ...
    interface: [
      ...(context.shellUpdatePending ? [row('shell-update', 'Обновить интерфейс', 'Готова новая версия. Перед обновлением незавершённая работа будет проверена.', 'rotate')] : []),
    ...
    personal: [
      ...
      row('mobile-access', 'На телефоне', 'Установка, виджеты Android, быстрый доступ и ярлыки.', 'smartphone'),
      row('reminders', 'Уведомления и напоминания', 'Когда и о чём напоминать лично вам.', 'bell'),
      row('finance', 'Личные финансы', 'Ваши личные счета, источники и распределение доходов.', 'fileText'),
      row('day', 'Границы дня', 'Начало и завершение дня для личного планирования.', 'clock'),
    ],
    workspace: [
      ...(!context.personal ? [row('team-finance', 'Финансы команды', 'Отдельный учёт команды или явное подключение команды-источника.', 'scale')] : []),

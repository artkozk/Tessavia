...
func (s *Server) handleListChatThreads(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.store.db.QueryContext(r.Context(), `
		SELECT t.id, CASE WHEN t.conversation_kind<>'' THEN t.conversation_kind ELSE t.kind END, t.title, t.record_id, COALESCE(rec.title, ''),
			...
			t.updated_at, (SELECT COUNT(*) FROM chat_members peers JOIN workspace_members wm ON wm.user_id=peers.user_id AND wm.workspace_id=t.workspace_id AND wm.status='active' WHERE peers.thread_id=t.id), personal.thread_id IS NOT NULL, archive.thread_id IS NOT NULL
		FROM chat_threads t
 ...
 LEFT JOIN chat_personal_archives archive ON archive.thread_id=t.id AND archive.user_id=cm.user_id
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Kind, &item.Title, &recordID, &item.RecordTitle, &item.LastMessage, &item.LastMessageAt, &item.UnreadCount, &item.PartnerUsername, &item.PartnerLastSeen, &item.UpdatedAt, &item.MemberCount, &item.Pinned, &item.Archived); err != nil {

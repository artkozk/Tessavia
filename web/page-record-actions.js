import { conditionConfig, updateConditionProperty, conditionMatches, conditionDescription } from './page-action-conditions.js?v=20260908-compound-action-2';
...
export async function openPageRecordAction(context,block,actionId,recordId,deps){
 ...
 apply.onclick=async()=>{if(!preview||preview.allowed===false||!active())return;apply.disabled=true;reload.disabled=true;const turn=++serial;status.textContent='Сохраняем изменение…';try{
  const record=await api(path,{method:'POST',headers,body:JSON.stringify({...body(),apply:true,expectedChangeCount:preview.changes?.length||1,expectedUpdatedAt:preview.expectedUpdatedAt,schemaHash:preview.schemaHash})});
  if(turn!==serial||!active()||!review.isConnected)return;

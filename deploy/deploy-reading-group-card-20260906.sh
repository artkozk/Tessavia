...
[[ "$RELEASE_NAME" =~ ^20260906-reading-groups-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-reading-groups
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-reading-groups-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
release_created=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-reading-groups-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/tessavie-reading-groups-check.XXXXXX)
...
check_http() {
  ...
  grep -qF 'styles.css?v=20260906-reading-groups-1' "$dry/index.html"
  grep -qF 'app.js?v=20260906-reading-groups-1' "$dry/index.html"
  curl --max-time 15 -fsS "$base/app.js?v=20260906-reading-groups-1" > "$dry/app.js"
  grep -qF "from './reading.js?v=20260906-reading-groups-1'" "$dry/app.js"
  grep -qF "from './outbox-ui.js?v=20260906-reading-groups-1'" "$dry/app.js"
  curl --max-time 15 -fsS "$base/reading.js?v=20260906-reading-groups-1" > "$dry/reading.js"
  grep -qF 'class="text-button" data-group-edit' "$dry/reading.js"
  grep -qF 'reading-group-card' "$dry/reading.js"
  curl --max-time 15 -fsS "$base/styles.css?v=20260906-reading-groups-1" > "$dry/styles.css"
  grep -qF 'grid-template-columns: repeat(auto-fill,minmax(280px,360px)); justify-content: start;' "$dry/styles.css"
  grep -qF '.reading-group-card.current { border-color: var(--line-strong); box-shadow: none; }' "$dry/styles.css"
  grep -qF 'grid-template-columns: repeat(2,minmax(0,1fr));' "$dry/styles.css"
  curl --max-time 15 -fsS "$base/sw.js" > "$dry/sw.js"
  grep -qF "tessavie-shell-20260906-reading-groups-1" "$dry/sw.js"

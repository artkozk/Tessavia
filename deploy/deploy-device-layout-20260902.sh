...
[[ "$RELEASE_NAME" =~ ^20260902-device-layout-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-device-layout
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
backup="/var/lib/business-control/backups/pre-device-layout-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/bizflow-device-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/bizflow-device-check.XXXXXX)
cp "$backup/business-control.db" "$dry/check.db"
mkdir "$dry/uploads"
chown -R business-control:business-control "$dry"
runuser -u business-control -- env BUSINESS_ADDRESS=127.0.0.1:18538 BUSINESS_DATABASE_PATH="$dry/check.db" BUSINESS_UPLOAD_PATH="$dry/uploads" "$release/business-control" > "$dry/server.log" 2>&1 &
dry_pid=$!
for attempt in $(seq 1 30); do
  if curl -fsS http://127.0.0.1:18538/api/health > "$dry/health.json" 2>/dev/null; then break; fi
  test "$attempt" -lt 30
  sleep 1
done
curl -fsS http://127.0.0.1:18538/ > "$dry/index.html"
grep -q '20260902-device-layout-1' "$dry/index.html"
...
test "$(sqlite3 "$dry/check.db" "SELECT COUNT(*) FROM schema_migrations WHERE version='022_device_interface_preferences.sql';")" = 1
kill "$dry_pid"
...
grep -q '20260902-device-layout-1' "$backup/local-index.html"
grep -q '20260902-device-layout-1' "$backup/public-index.html"
test "$(systemctl is-active business-control)" = active
test "$(sqlite3 "$db" 'PRAGMA integrity_check;')" = ok
test "$(sqlite3 "$db" 'SELECT COUNT(*) FROM pragma_foreign_key_check;')" = 0
test "$(sqlite3 "$db" "SELECT COUNT(*) FROM schema_migrations WHERE version='022_device_interface_preferences.sql';")" = 1

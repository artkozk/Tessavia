...
marker='[request:page-forms-20260908]'
try:
 records=api('/records')
 ...
 item=found[0] if found else api('/records','POST',{'type':'task','title':'P0 · Собирать переносимые формы создания записей','description':marker+' Пользователь добавляет блок формы, выбирает доску, поля и их порядок, переименовывает подписи, меняет ширины/цвета/шрифт, убирает основные и пользовательские поля. Если название скрыто, задаёт явное начальное название; скрытое обязательное поле без начального значения отклоняется до сохранения. Форма создаёт настоящую запись через существующие права/валидацию, сохраняет ввод при ошибках/reload и не отправляется автоматически. Предпросмотр не создаёт данные. Набор переносит форму и независимую схему с переназначением полей. Приёмка: ручная сборка формы обращения, создание, черновик, шаблон и копия, 320px. Изменение существующих записей, вычисляемые формулы, условные действия и все встроенные элементы остаются следующими этапами.','ownerId':user[0],'parentId':'26c54f79aa1c9110536670ae8371a868','priority':'high','workstream':'platform','status':'in_progress','editPolicy':'shared'})
 receipt={'task':item['id'],'title':item['title'],'status':api('/records/'+item['id'])['record']['status']}
 Path('/tmp/tessavie-page-form-task.json').write_text(json.dumps(receipt,ensure_ascii=False,indent=2));print(json.dumps(receipt,ensure_ascii=False))
finally:

...
[[ "$RELEASE_NAME" =~ ^20260904-personal-reminders-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-personal-reminders
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-personal-reminders-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-personal-reminders-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-personal-reminders-check.XXXXXX)
...
python3 /tmp/verify-personal-reminders-migration.py "$backup/business-control.db" "$dry/check.db"

check_http() {
  local base="$1"
  curl --max-time 15 -fsS "$base/reminder-settings.js?v=20260904-personal-reminders-2" > "$dry/reminder-settings.js"
  grep -qF 'personalEnabled' "$dry/reminder-settings.js"
  ...
  curl --max-time 15 -fsS "$base/personal-reminders.js?v=20260904-personal-reminders-2" > "$dry/personal-reminders.js"
  ...
  grep -q '20260904-personal-reminders-2' "$dry/index.html"
  ...
  grep -qF "20260904-personal-reminders-2" "$dry/sw.js"

...
[[ "$RELEASE_NAME" =~ ^20260902-note-sheet-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-note-sheet
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
backup="/var/lib/business-control/backups/pre-note-sheet-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/bizflow-note-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/bizflow-note-check.XXXXXX)
...
grep -q '20260902-note-sheet-1' "$dry/index.html"
...
grep -q '20260902-note-sheet-1' "$backup/local-index.html"
grep -q '20260902-note-sheet-1' "$backup/public-index.html"

...
[[ "$RELEASE_NAME" =~ ^20260907-chat-list-search-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-chat-list-search
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-chat-list-search-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
release_created=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-chat-list-search-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/tessavie-chat-list-search-check.XXXXXX)
...
python3 /tmp/verify-chat-list-search-migration.py "$backup/business-control.db" "$dry/check.db"

check_http() {
  local base="$1"
  curl --max-time 15 -fsS "$base/" > "$dry/index.html"
  grep -qF 'app.js?v=20260907-chat-list-search-2' "$dry/index.html"
  grep -qF 'styles.css?v=20260907-chat-list-search-2' "$dry/index.html"
  curl --max-time 15 -fsS "$base/app.js?v=20260907-chat-list-search-2" > "$dry/app.js"
  grep -qF 'chat-workspace.js?v=20260907-chat-list-search-2' "$dry/app.js"
  grep -qF 'data-chat-outbox' "$dry/app.js"
  grep -qF 'data-planner-journal' "$dry/app.js"
  grep -qF 'applyChatListFilter' "$dry/app.js"
  grep -qF 'collection-default-config' "$dry/app.js"
  curl --max-time 15 -fsS "$base/chat-workspace.js?v=20260907-chat-list-search-2" > "$dry/chat.js"
  grep -qF 'createChatWorkspaceUI' "$dry/chat.js"
  curl --max-time 15 -fsS "$base/chat-groups.js?v=20260907-chat-list-search-2" > "$dry/groups.js"
  grep -qF 'createChatGroupUI' "$dry/groups.js"
  curl --max-time 15 -fsS "$base/personal-calendar.js?v=20260907-chat-list-search-2" > "$dry/calendar.js"
  grep -qF 'openRecurrence' "$dry/calendar.js"
  grep -qF 'bindPersonalRecurrenceScope' "$dry/app.js"
  curl --max-time 15 -fsS "$base/personal-today.js?v=20260907-chat-list-search-2" > "$dry/today.js"
  ...
  curl --max-time 15 -fsS "$base/styles.css?v=20260907-chat-list-search-2" > "$dry/styles.css"
  grep -qF -- '--chat-menu-left' "$dry/styles.css"
  curl --max-time 15 -fsS "$base/sw.js" > "$dry/sw.js"
  grep -qF 'tessavie-shell-20260907-chat-list-search-2' "$dry/sw.js"

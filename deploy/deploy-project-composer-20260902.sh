...
[[ "$RELEASE_NAME" =~ ^20260902-project-composer-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-project-composer
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
backup="/var/lib/business-control/backups/pre-project-composer-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/bizflow-composer-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/bizflow-composer-check.XXXXXX)
...
grep -q '20260902-project-composer-1' "$dry/index.html"
...
test "$(sqlite3 "$dry/check.db" "SELECT COUNT(*) FROM schema_migrations WHERE version='021_project_composer.sql';")" = 1
kill "$dry_pid"
...
grep -q '20260902-project-composer-1' "$backup/local-index.html"
grep -q '20260902-project-composer-1' "$backup/public-index.html"
test "$(systemctl is-active business-control)" = active
test "$(sqlite3 "$db" 'PRAGMA integrity_check;')" = ok
test "$(sqlite3 "$db" 'SELECT COUNT(*) FROM pragma_foreign_key_check;')" = 0
test "$(sqlite3 "$db" "SELECT COUNT(*) FROM schema_migrations WHERE version='021_project_composer.sql';")" = 1

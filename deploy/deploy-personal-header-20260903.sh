...
[[ "$RELEASE_NAME" =~ ^20260903-personal-header-[a-f0-9]+$ ]]
...
dry=$(mktemp -d /tmp/bizflow-personal-header-check.XXXXXX)
...
curl -fsS http://127.0.0.1:18543/ > "$dry/index.html"
grep -q '20260903-personal-header-1' "$dry/index.html"
curl -fsS http://127.0.0.1:18543/app.js > "$dry/app.js"
curl -fsS http://127.0.0.1:18543/styles.css > "$dry/styles.css"
grep -qF '.personal-create-menu { position: relative; align-self: auto; }' "$dry/styles.css"
grep -qF '.personal-heading > [data-personal-calendar] { min-height: 44px; white-space: nowrap; }' "$dry/styles.css"
test "$(curl -s -X PUT -o /dev/null -w '%{http_code}' http://127.0.0.1:18543/api/records/check/collection)" = 401
test "$(curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:18543/api/personal/overview)" = 401
...
grep -q '20260903-personal-header-1' "$backup/local-index.html"
grep -q '20260903-personal-header-1' "$backup/public-index.html"

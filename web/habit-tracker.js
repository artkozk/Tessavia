...
export function createHabitUI(ctx) {
  ...
  const reminded = new Set();let refreshingDay = false;
  function remind() {
    ...
    if (staleDay && !refreshingDay && navigator.onLine) { refreshingDay = true;void loadPersonal({force:true}).finally(() => {refreshingDay=false;});return; }
    for (const h of state.personal?.habits || []) {
      if (h.archivedAt || h.paused || !h.rule.reminderTime) continue;
      const day = h.days?.find(d => d.date === h.today);
      if (!day?.editable || !['pending','partial','snoozed'].includes(day.state)) continue;
      const now = new Date(), parts = new Intl.DateTimeFormat('en-CA',{timeZone:h.timezone,year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).formatToParts(now);
      const part = key => parts.find(p => p.type===key)?.value;
      if (`${part('year')}-${part('month')}-${part('day')}` !== h.today || `${part('hour')}:${part('minute')}` < h.rule.reminderTime) continue;
      if (day.state==='snoozed' && now - new Date(day.checkin.updatedAt) < 3600000) continue;
      const key = `habit-reminder:${state.me.id}:${h.id}:${h.today}:${day.state==='snoozed' ? day.checkin.updatedAt : ''}`;
      if (reminded.has(key)) continue;
      try { if (localStorage.getItem(key)) continue;localStorage.setItem(key,'1'); } catch (_) {}
      reminded.add(key);toast('Пора отметить результат привычки в личном пространстве');
    }
  }
  setInterval(remind, 60000);document.addEventListener('visibilitychange',remind);
  ...
  function renderRow(h, compact = false) {
    ...
    return `<article class="habit-card ${compact ? 'compact' : ''}" data-habit-color="${e(h.colorKey)}"><div class="habit-card-heading"><span class="habit-emblem">${icon(h.iconKey || 'checkSquare')}</span><button class="habit-title text-button" type="button" data-habit-open="${h.id}"><strong>${e(h.title)}</strong><span>${e(habitModes[r.mode])} · ${e(habitScheduleLabel(r))}</span></button><button type="button" class="icon-button" data-habit-settings="${h.id}" aria-label="Настроить привычку ${e(h.title)}">${icon('settings')}</button></div><div class="habit-goal"><span>${e(goalLabel(r))}</span><strong>${h.archivedAt ? 'В архиве' : today ? e(habitStateLabel(today)) : 'Вне расписания'}</strong></div><div class="habit-week" aria-label="Последние семь дней">${(h.days || []).map(d => `<button type="button" class="habit-cell is-${d.state}" data-habit-date="${d.date}" data-habit-open="${h.id}" aria-label="${e(dateLabel(d.date))}: ${e(habitStateLabel(d))}" title="${e(dateLabel(d.date))}: ${e(habitStateLabel(d))}"><small>${weekdays[new Date(`${d.date}T12:00:00Z`).getUTCDay()]}</small><b>${symbols[d.state]}</b></button>`).join('')}</div><p class="personal-muted" data-habit-pending="${h.id}" hidden></p><footer class="habit-card-footer"><span>Серия ${h.currentStreak} · лучшая ${h.bestStreak}<small>${e(h.summary?.streakUnit || 'плановых дней')}</small></span>${quick && (r.mode === 'build' || r.mode === 'quit') ? `<button type="button" class="secondary" data-habit-quick="${h.id}">${r.mode === 'quit' ? 'День без действия' : 'Выполнено'}</button>` : `<button type="button" class="secondary" data-habit-open="${h.id}" data-habit-date="${h.today}">${today?.editable ? 'Записать результат' : 'Открыть трекер'}</button>`}</footer></article>`;
  }
  function renderList(habits) {
    ...
    return `<section class="personal-section habit-section"><div class="section-heading"><div><p class="eyebrow">Только для вас</p><h2>Привычки и трекер</h2></div><button type="button" class="primary" data-habit-new>${icon('plus')} Добавить</button></div><p class="personal-muted">Отмечайте факт, наблюдайте ритм и меняйте цель постепенно. Пропущенная отметка не означает срыв.</p><div class="habit-filters" role="group" aria-label="Фильтр привычек">${[['today', 'Сегодня'], ['all', 'Все'], ['build', 'Делать'], ['anti', 'Отказ и сокращение'], ['paused', 'Пауза'], ['archive', 'Архив']].map(([value, label]) => `<button type="button" class="secondary" aria-pressed="${filter === value}" data-habit-filter="${value}">${label}</button>`).join('')}</div><p class="panel-note">${shown.length} привычек</p><div class="habit-cards">${shown.map(h => renderRow(h)).join('') || '<p class="personal-muted">В этом разделе пока нет привычек. Можно выбрать «Все» или добавить свою.</p>'}</div></section>`;
  }
  function bind(root = document) {
    void pendingBadges();remind();
  ...
  function drawTracker(data, selectedDate, period, offset) {
    ...
    show(`${header(h.title, `${habitModes[h.rule.mode]} · ${h.timezone}`)}<div class="habit-tracker"><div class="habit-tracker-intro"><p>${e(h.description || goalLabel(h.rule))}</p><button type="button" class="secondary" data-settings>${icon('settings')} Настройки</button><button type="button" class="text-button" data-links>${icon('link')} Связать с целью</button></div><div class="habit-tracker-controls"><label>Период<select data-period>${[['week', 'Неделя'], ['month', 'Месяц'], ['quarter', '12 недель']].map(([v, t]) => option(v, t, period)).join('')}</select></label><div class="habit-period-navigation"><button type="button" class="icon-button" data-back aria-label="Предыдущий период">${icon('chevronLeft')}</button><strong>${e(dateLabel(data.from))} — ${e(dateLabel(data.to))}</strong><button type="button" class="icon-button" data-forward aria-label="Следующий период" ${offset >= 0 ? 'disabled' : ''}>${icon('chevronRight')}</button></div><button type="button" class="text-button" data-now>К текущему периоду</button></div><div class="habit-stats"><div class="habit-progress"  role="img" aria-label="${s.percent == null ? 'Нет завершённого плана' : `${s.percent}% выполнения плана`}"><svg viewBox="0 0 100 100" aria-hidden="true"><circle class="habit-ring-track" cx="50" cy="50" r="44"/><circle class="habit-ring-value" cx="50" cy="50" r="44" pathLength="100" stroke-dasharray="100" stroke-dashoffset="${100 - (s.percent || 0)}"/></svg><strong>${s.percent == null ? '—' : `${s.percent}%`}</strong></div><div><strong>${s.percent == null ? 'Пока нет завершённого плана' : quota ? 'От завершённых периодов' : 'От прошедших плановых дней'}</strong><p>${s.success} успешно · ${s.partial} частично · ${s.failed} не выполнено<br>${s.unmarked} без отметки · ${s.skipped} пропущено · ${s.paused} на паузе</p><small>Будущее, отдых и пропуски исключены. Незавершённый сегодня день ждёт результата.</small></div><div><strong>${s.currentStreak} / ${s.bestStreak}</strong><p>Текущая / лучшая серия<br>${e(s.streakUnit)}, за историю этого ритма</p></div></div>${['quit', 'reduce', 'quantity', 'duration'].includes(h.rule.mode) ? `<p class="habit-measure-summary">Факт за период: <b>${s.total} ${e(h.rule.unit)}</b>${['quit', 'reduce'].includes(h.rule.mode) ? ` · ${h.rule.mode === 'quit' ? 'Дней без действия' : 'Дней в пределах лимита'}: ${s.success} · ${h.rule.mode === 'quit' ? 'Дней со срывом' : 'Превышений'}: ${s.failed}${s.lastLapse ? ` · Последний случай: ${e(dateLabel(s.lastLapse))}` : ''}` : ''}</p>` : ''}${data.periods.length ? `<div class="habit-periods">${data.periods.map(p => `<div><span>${e(dateLabel(p.start))} — ${e(dateLabel(p.end))}</span><strong>${p.actual} / ${p.target} ${e(p.measure === 'days' ? 'дней' : h.rule.unit)}</strong><small>${p.state === 'success' ? 'Цель достигнута' : p.state === 'failed' ? 'Период завершён ниже цели' : p.state === 'rest' ? 'Нет плана' : 'Период продолжается'}</small></div>`).join('')}</div>` : ''}<div class="habit-calendar" aria-label="Календарь привычки">${[1, 2, 3, 4, 5, 6, 0].map(i => `<span class="habit-calendar-weekday">${weekdays[i]}</span>`).join('')}${'<span></span>'.repeat(padding)}${calendar.map(d => `<button type="button" class="habit-calendar-day is-${d.state} ${d.date === h.today ? 'is-today' : ''}" data-day="${d.date}" aria-label="${e(dateLabel(d.date))}: ${e(habitStateLabel(d))}"><span>${Number(d.date.slice(-2))}</span><b>${symbols[d.state]}</b>${d.checkin && d.checkin.state === 'measured' ? `<small>${d.checkin.value}</small>` : ''}${d.checkin?.note ? '<i aria-label="Есть заметка">•</i>' : ''}</button>`).join('')}</div><div class="habit-legend">${['success', 'partial', 'failed', 'pending', 'skipped', 'paused', 'moved'].map(k => `<span><b class="is-${k}">${symbols[k]}</b>${e(habitStateLabel({ state: k, rule: h.rule }))}</span>`).join('')}</div><div data-day-editor></div><details class="habit-details"><summary>Ритм по дням недели</summary><div class="habit-weekday-stats">${[1, 2, 3, 4, 5, 6, 0].map(i => `<span><strong>${weekdays[i]}</strong>${s.weekdays[i].success} / ${s.weekdays[i].planned}<small>${s.weekdays[i].planned ? `${Math.round(s.weekdays[i].success / s.weekdays[i].planned * 100)}%` : 'Нет данных'}</small></span>`).join('')}</div><p class="personal-muted">Успешные / прошедшие плановые дни. Малое количество наблюдений ещё не показывает устойчивый ритм.</p></details><details class="habit-details"><summary>История целей и пауз</summary><p><a class="text-button" href="/api/personal/habits/${h.id}/export" download>Скачать личную историю (JSON)</a></p>${h.rules.map(r => `<p>С ${e(dateLabel(r.effectiveDate))}: ${e(goalLabel(r))} · ${e(habitScheduleLabel(r))}${r.endDate ? ` · до ${e(dateLabel(r.endDate))}` : ''}</p>`).join('')}${h.pauses.filter(p => !p.endDate || p.endDate >= p.startDate).map(p => `<p>${p.reason === 'archive' ? 'Архив' : 'Пауза'}: ${e(dateLabel(p.startDate))} — ${p.endDate ? e(dateLabel(p.endDate)) : 'до возобновления'}</p>`).join('')}</details><div class="habit-lifecycle">${h.archivedAt ? '<button type="button" class="primary" data-lifecycle="restore">Восстановить привычку</button>' : `${h.paused ? '<button type="button" class="secondary" data-lifecycle="resume">Возобновить сегодня</button>' : '<label>Пауза до (необязательно)<input type="date" data-pause-end></label><button type="button" class="secondary" data-lifecycle="pause">Поставить на паузу</button>'}<button type="button" class="danger-text" data-lifecycle="archive">В архив</button>`}</div></div>`);
    const root = content();
  ...
  function settings(h = null) {
    ...
    form.addEventListener('submit', async event => {
      ...
      const f = new FormData(form);const rule = { mode: h?.rule.mode || f.get('mode'), cadence: f.get('cadence'), target: Number(f.get('target')), unit: f.get('unit'), interval: Number(form.elements.interval.value), weekdays: [...form.querySelectorAll('[name="weekdays"]:checked')].map(input => Number(input.value)), periodTarget: Number(form.elements.periodTarget.value), periodMeasure: form.elements.periodMeasure.value, endDate: f.get('endDate'), reminderTime: f.get('reminderTime'), effectiveDate: f.get('effectiveDate') || '' };

"""Compare every existing table across migration 041 without printing private rows."""
...
assert tables(after)-tables(before)=={'personal_publications'}
...
assert after.execute('SELECT COUNT(*) FROM personal_publications').fetchone()[0]==0
...
assert current-previous=={'041_personal_publications.sql'} and not previous-current

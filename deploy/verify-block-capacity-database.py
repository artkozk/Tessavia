...
Startup must preserve every table value. A previous binary may only run when
there are no pending archived trees: it cannot enforce their reserved IDs.
Validation accepts active trash; rollback deliberately does not. No database
...
class PendingBlockTrash(ValueError):
    pass
...
def rollback_safe(path):
    with closing(connect_readonly(path)) as db:
        if validate(db):
            raise PendingBlockTrash('Pending block trash requires the current binary')
    print('ROLLBACK_SAFE=schema_070_no_pending_block_trash_latest_data_retained')
...
if __name__ == '__main__':
    if len(sys.argv) == 3 and sys.argv[1] == '--rollback-safe':
        ...
        except PendingBlockTrash:
            print('ROLLBACK_BLOCKED=pending_block_trash_keep_candidate_and_latest_data', file=sys.stderr)
            raise SystemExit(3)
        except Exception:
        ...
        raise SystemExit('Usage: verify-block-capacity-database.py BEFORE AFTER | --validate-only DB | --rollback-safe DB')

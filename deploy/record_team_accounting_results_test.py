...
SPEC = importlib.util.spec_from_file_location('team_accounting_recorder', Path(__file__).with_name('record-team-accounting-results-20260915.py'))
REC = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(REC)


class TeamAccountingRecorderChecks(unittest.TestCase):
    def test_registered_two_task_ids_and_parent_are_fixed(self):
        self.assertEqual(REC.TASK_IDS, {'4c262b5f62a29ef17162809ec83fc1d7', '26e949d20d9e18b96819c000d1f420d1'})
        self.assertEqual(len(REC.TASKS), 2)
        self.assertEqual(REC.PARENT_ID, '3d81cdb847c8fcfdb329639c22f3067a')
    ...
    def setUp(self):
        ...
        self.acceptance = {'goTests': True, 'goVet': True, 'migrationPreserved': True, 'rollbackTrial': True,
            'goTestCount': 467, 'nodeTests': 705, 'buildInputs': 540, 'verifierTests': 18,
        ...
        self.identity = ('/opt/business-control/releases/20260915-team-accounting-aaaaaaa', 'a' * 40,
                         'b' * 64, '/var/lib/business-control/backups/pre-team-accounting-20260915T100000Z')
    ...
    def test_missing_checks_and_invented_physical_acceptance_are_rejected(self):
        ...
        for key, value in (('goTests', False), ('goTestCount', True), ('goTestCount', 466),
                           ('nodeTests', 704), ('verifierTests', 17), ('rollbackTrial', False),
    ...
    def test_exact_release_commit_and_backup_prefix_are_bound(self):
        ...
        for index, value in ((0, '/opt/business-control/releases/20260915-smart-select-aaaaaaa'),
                             (0, '/opt/business-control/releases/20260915-team-accounting-aaaaaaa/../other'),
    ...
    def test_generated_evidence_uses_actual_widths_counts_and_exactly_the_two_ids(self):
        ...
        for text in evidence.values():
            self.assertIn('467 Go-тестовых событий', text)
            self.assertIn('705 Node-тестов', text)
            self.assertIn('Браузерные ширины 320,390,1280 px', text)
            self.assertIn('077/150 таблиц', text)
    ...
    def test_prior_release_tasks_cannot_be_read_or_written(self):
        for task_id in ('ad787975b390e40313472673274982d4', '16fbc52ae06d4d296dff09e93f3319ba', '9c0795b89d32595e7350dd3eca14fcd2', '22bfe15d74ab5a9b92510f1a03fdede5'):
...
class TeamAccountingReleaseChecks(unittest.TestCase):

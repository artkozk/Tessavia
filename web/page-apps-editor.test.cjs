...
function harness(){
 ...
 const context=vm.createContext({structuredClone,crypto:{randomUUID:()=> `00000000-0000-4000-8000-${String(++nextID).padStart(12,'0')}`},localStorage:{getItem(key){reads.push(key);return storage.get(key)||null;},setItem(key,value){storage.set(key,value);},removeItem(key){storage.delete(key);}},
  createPageFinanceUI:()=>({mount(){}}),updatePageFinanceConfig:()=>false,createPageMediaUI:()=>({mount(){}}),createPageDataUI:()=>({mount(){}}),mountPageForms(){},applyElementStyles(){},migrateFormElementStyles(){},blockVisible:()=>true,blockOutline:def=>def.blocks.map(block=>({block,depth:0})),groupChoices:()=>[],elementStyleConfig:()=>'',visibilityConfig:()=>''});

...
[[ "$RELEASE_NAME" =~ ^20260904-reminder-digests-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-reminder-digests
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-reminder-digests-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-reminder-digests-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-reminder-digests-check.XXXXXX)
...
python3 /tmp/verify-reminder-digests-migration.py "$backup/business-control.db" "$dry/check.db"
...
check_http() {
  ...
  grep -q '20260904-reminder-digests-1' "$dry/index.html"
  ...
  grep -qF "20260904-reminder-digests-1" "$dry/sw.js"

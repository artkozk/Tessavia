...
ASSETS = '20260914-constructor-safety-1'
MARKER = '[release:constructor-safety-20260914]'
ROOT_ID = '66da4e412e714f4ea85c56ff85de03ec'
SPECS = ({'id': 'db7706ead51cbb2a9c2514ae568a297d', 'requestMarker': '[request:component-drafts-20260914]', 'title': 'P1 · Не терять черновик названия и описания своего блока', 'result': 'Название и описание своего блока сохраняются в черновике пользователя, пространства, страницы и корневого блока. Проверены возврат, закрытие, повторное открытие и ошибки; неизвестный результат повторяется прежним запросом. Сохраняется лишь подтверждённый узкий результат, библиотека и конструктор целиком не объявляются завершёнными.'}, {'id': '16d7b9697374957b6876208c54b1f90e', 'requestMarker': '[request:block-kind-safety-20260914]', 'title': 'P0 · Не удалять действия незаметно при смене типа блока', 'result': 'Смена типа списка с настроенными действиями перехватывается до изменения черновика. Полная конфигурация сохраняется, рядом с выбором типа показана причина и путь к добавлению отдельного блока. Без действий обычная смена типа доступна. Серверный контракт и схема базы не менялись; автоматический перенос действий в другой тип не реализован.'})
...
def main():
    ...
    match = re.fullmatch(r'/opt/business-control/releases/20260914-constructor-safety-([a-f0-9]{7,40})', release)
    ...
    assert backup_path.name.startswith('pre-constructor-safety-')
    ...
    assert (backup_path / 'previous-release.txt').read_text().strip() == '/opt/business-control/releases/20260914-field-conversion-ae9402f'
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            'release_database_verifier', Path(__file__).with_name('verify-constructor-safety-database.py'))
        ...
        common = (
            ...
            'Документы: docs/operations/CONSTRUCTOR_SAFETY_2026_09_14.md; '
            'docs/operations/CONSTRUCTOR_SAFETY_RELEASE_SAFETY_2026_09_14.md. '
            'Исправления работают в редакторе; реальные пользовательские страницы на production для проверки не менялись. '
        ...
        Path('/tmp/constructor-safety-receipt.json').write_text(

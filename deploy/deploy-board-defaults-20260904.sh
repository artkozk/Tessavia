...
[[ "$RELEASE_NAME" =~ ^20260904-board-defaults-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-board-defaults
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-board-defaults-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-board-defaults-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-board-defaults-check.XXXXXX)
...
check_http() {
  ...
  grep -q '20260904-board-defaults-1' "$dry/index.html"
  ...
  grep -qF "20260904-board-defaults-1" "$dry/sw.js"

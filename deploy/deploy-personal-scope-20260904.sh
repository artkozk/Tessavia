...
[[ "$RELEASE_NAME" =~ ^20260904-personal-scope-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-personal-scope
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-personal-scope-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-personal-scope-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-personal-scope-check.XXXXXX)
...
python3 /tmp/verify-personal-scope-migration.py "$backup/business-control.db" "$dry/check.db"
...
check_http() {
  ...
  grep -q '20260904-personal-scope-1' "$dry/index.html"
  ...
  grep -qF "20260904-personal-scope-1" "$dry/sw.js"

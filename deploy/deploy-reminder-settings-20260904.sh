...
[[ "$RELEASE_NAME" =~ ^20260904-reminder-settings-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-reminder-settings
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-reminder-settings-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-reminder-settings-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-reminder-settings-check.XXXXXX)
...
python3 /tmp/verify-reminder-settings-migration.py "$backup/business-control.db" "$dry/check.db"

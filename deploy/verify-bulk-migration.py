"""Verify migration 038 on a server-local copy without disclosing personal facts."""
...
assert tables(after)-tables(before)=={'record_batches'}
...
for table in tables(before):
    ...
    if table=='schema_migrations':
        ...
        assert after.execute("SELECT COUNT(*) FROM schema_migrations WHERE version='038_record_batches.sql'").fetchone()[0]==1
    else: assert old==new,'Existing data changed in '+table
assert after.execute('SELECT COUNT(*) FROM record_batches').fetchone()[0]==0

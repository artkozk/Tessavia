...
type PageRecordAction struct {
	Changes       []PageActionChange   `json:"changes,omitempty"`
...
func validatePageRecordActions(b PageAppBlock) error {
	...
	for _, a := range b.Actions {
		if len(a.Changes) > 7 {
			return errors.New("В одной кнопке доступно до 8 изменений")
		}
		targets := map[string]bool{}
		for _, step := range actionChanges(a) {
			if targets[step.FieldID] {
				return errors.New("Одно поле нельзя изменять дважды в одной кнопке")
			}
			targets[step.FieldID] = true
			if err := validateSinglePageAction(step); err != nil {
				return err
			}
		}
		if seen[a.ID] {
			return errors.New("Ключи действий должны быть разными")
		}
		seen[a.ID] = true
	}
	return nil
...
func (s *Server) validatePageActionSource(ctx context.Context, b PageAppBlock, fields []CollectionField) error {
	...
	for _, parent := range b.Actions {
		for _, a := range actionChanges(parent) {
...
func (s *Server) handlePageRecordAction(w http.ResponseWriter, r *http.Request) {
	...
	changes, schema, err := s.previewActionChanges(r.Context(), action, record, source.Fields)
	if err != nil {
		writeError(w, 409, err.Error())
		return
	}
	if input.Apply && len(changes) > 1 && input.ExpectedChangeCount != len(changes) {
	...
	for _, f := range source.Fields {
		if action.Condition != nil && f.ID == action.Condition.FieldID {
			conditionField = &f
		}
	}
	...
	for _, change := range changes {
		if change.empty {
			_, err = tx.ExecContext(r.Context(), `DELETE FROM record_field_values WHERE record_id=? AND field_id=?`, record.ID, change.Field.ID)
		} else {
			_, err = tx.ExecContext(r.Context(), `INSERT INTO record_field_values(record_id,field_id,value_json,updated_by,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(record_id,field_id) DO UPDATE SET value_json=excluded.value_json,updated_by=excluded.updated_by,updated_at=excluded.updated_at`, record.ID, change.Field.ID, string(change.After), currentUser(r).ID, now)
		}
		if err != nil {
			writeError(w, 500, "Не удалось сохранить изменения. Ни одно поле не изменено")
			return
		}
	}

	if err = writeActivity(r.Context(), tx, currentUser(r).ID, record.Type, record.ID, "custom_fields_updated", "Действие: "+action.Label, map[string]any{"pageId": r.PathValue("id"), "actionId": action.ID, "fieldId": field.ID, "sourceFieldId": action.SourceFieldID, "before": preview.Before, "after": preview.After, "changes": changes}); err != nil {

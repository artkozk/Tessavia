...
[[ "$RELEASE_NAME" =~ ^20260904-waiting-ping-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-waiting-ping
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-waiting-ping-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-waiting-ping-check.* ]]; then rm -rf -- "$dry"; fi
...
python3 /tmp/verify-waiting-ping-migration.py "$backup/business-control.db" "$dry/check.db"
...
check_http() {
  ...
  grep -q '20260904-waiting-ping-1' "$dry/index.html"
  ...
  grep -qF "20260904-waiting-ping-1" "$dry/sw.js"

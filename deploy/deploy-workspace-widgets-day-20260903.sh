...
[[ "$RELEASE_NAME" =~ ^20260903-workspace-widgets-day-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-workspace-widgets-day
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
backup="/var/lib/business-control/backups/pre-workspace-widgets-day-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''; dry_pid=''; switch_started=0
cleanup() {
  result=$?
  trap - EXIT
  if [[ -n "$dry_pid" ]]; then kill "$dry_pid" 2>/dev/null || true; wait "$dry_pid" 2>/dev/null || true; fi
  if [[ -n "$dry" && "$dry" == /tmp/bizflow-workspace-widgets-day-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/bizflow-workspace-widgets-day-check.XXXXXX)
cp "$backup/business-control.db" "$dry/check.db"
mkdir "$dry/uploads"
chown -R business-control:business-control "$dry"
runuser -u business-control -- env BUSINESS_ADDRESS=127.0.0.1:18542 BUSINESS_DATABASE_PATH="$dry/check.db" BUSINESS_UPLOAD_PATH="$dry/uploads" "$release/business-control" > "$dry/server.log" 2>&1 &
dry_pid=$!
for attempt in $(seq 1 30); do
  if curl -fsS http://127.0.0.1:18542/api/health > "$dry/health.json" 2>/dev/null; then break; fi
  test "$attempt" -lt 30; sleep 1
done
curl -fsS http://127.0.0.1:18542/ > "$dry/index.html"
grep -q '20260903-workspace-widgets-day-1' "$dry/index.html"
curl -fsS http://127.0.0.1:18542/app.js > "$dry/app.js"
grep -q 'function renderDayWorkspace' "$dry/app.js"
grep -q 'function openWidgetLibrary' "$dry/app.js"
test "$(curl -s -X PUT -o /dev/null -w '%{http_code}' http://127.0.0.1:18542/api/records/check/collection)" = 401
test "$(curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:18542/api/personal/overview)" = 401
...
test "$(sqlite3 "$dry/check.db" "SELECT COUNT(*) FROM schema_migrations WHERE version='026_personal_note_schedule.sql';")" = 1
...
test "$(sqlite3 "$dry/check.db" "SELECT COUNT(*) FROM personal_notes WHERE scheduled_date IS NOT NULL;")" = 0
...
grep -q '20260903-workspace-widgets-day-1' "$backup/local-index.html"
grep -q '20260903-workspace-widgets-day-1' "$backup/public-index.html"
test "$(systemctl is-active business-control)" = active
test "$(sqlite3 "$db" 'PRAGMA integrity_check;')" = ok
test "$(sqlite3 "$db" 'SELECT COUNT(*) FROM pragma_foreign_key_check;')" = 0
test "$(sqlite3 "$db" "SELECT COUNT(*) FROM schema_migrations WHERE version='026_personal_note_schedule.sql';")" = 1

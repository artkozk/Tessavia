"""Compare every existing table across migration 042 without printing private rows."""
...
assert tables(after)-tables(before)=={'user_journey_preferences'}
...
assert after.execute('SELECT COUNT(*) FROM user_journey_preferences').fetchone()[0]==0
...
assert current-previous=={'042_journey_preferences.sql'} and not previous-current

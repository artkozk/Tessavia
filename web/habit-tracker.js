...
export function habitQuickPayload(habit, day) {
  const rule = day?.rule || habit?.rule;
  if (!habit || !day?.editable || !isSimpleHabit(rule) || !['pending', 'snoozed'].includes(day.state)) return null;
  return { state: 'measured', value: rule.mode === 'quit' ? 0 : 1, note: day.checkin?.note || '', expectedUpdatedAt: day.checkin?.updatedAt || '', revision: habit.revision };
}

export function habitMeasureLabel(rule) {
  if (rule.mode === 'duration') return 'Сколько минут за день?';
  if (rule.mode === 'reduce') return `Сколько за день, ${rule.unit}?`;
  return `Количество за день, ${rule.unit}`;
}

// A failed/skipped day carries no measurement. Its storage projection of zero
// must not look like a known zero in an editable number field.
export function habitMeasuredValue(checkin) {
  return checkin?.state === 'measured' && Number.isFinite(checkin.value) ? checkin.value : '';
}
...
export function createHabitUI(ctx) {
  ...
  const goalLabel = r => r.mode === 'quit' ? 'День без нежелательного действия' : r.mode === 'build' ? 'Выполнить действие' : r.mode === 'reduce' ? `Не больше ${r.target} ${r.unit}` : `Цель: ${r.target} ${r.unit}`;
  ...
  const quickSaving = new Set();
  function rememberPosition(button) {
    const row = button.closest?.('[data-habit-row]');
    if (!row || typeof window.scrollBy !== 'function') return () => {};
    const owner = state.me?.id, page = state.view, workspace = state.activeWorkspaceId, tab = state.personalTab;
    const top = row.getBoundingClientRect().top;
    const ledger = row.closest('.habit-ledger, .habit-list');
    const ledgerTop = ledger?.scrollTop;
    let interrupted = false;
    const interrupt = event => { if (event.type !== 'keydown' || ['ArrowUp', 'ArrowDown', 'PageUp', 'PageDown', 'Home', 'End', ' '].includes(event.key)) interrupted = true; };
    for (const type of ['wheel', 'touchmove', 'keydown', 'resize']) window.addEventListener(type, interrupt, { passive: true });
    const dispose = () => { for (const type of ['wheel', 'touchmove', 'keydown', 'resize']) window.removeEventListener(type, interrupt); };
    const restore = keepFocus => {
      dispose();
      if (owner !== state.me?.id || page !== state.view || workspace !== state.activeWorkspaceId || tab !== state.personalTab || interrupted) return;
      const next = [...document.querySelectorAll('[data-habit-row]')].find(item => item.dataset.habitRow === row.dataset.habitRow);
      if (!next) return;
      // Desktop keeps a scrollable list. Its replacement starts at zero, so
      // restore the list first; moving only the window cannot reveal its row.
      const nextLedger = next.closest('.habit-ledger, .habit-list');
      if (ledger && nextLedger) nextLedger.scrollTop = ledgerTop;
      // Reflow after the status changes should not pull the row away from a
      // finger or keyboard focus. A deliberate user scroll takes precedence.
      const delta = next.getBoundingClientRect().top - top;
      if (Math.abs(delta) > 1) window.scrollBy({ top: delta, behavior: 'instant' });
      if (keepFocus) {
        const date = button.dataset.habitDate;
        const nextButton = [...next.querySelectorAll('button')].find(item => date ? item.dataset.habitDate === date : item.closest('.habit-entry-action'));
        nextButton?.focus({ preventScroll: true });
      }
    };
    restore.dispose = dispose;
    return restore;
  }
  async function quickCheckin(button) {
    const owner = state.me?.id, h = findHabit(button.dataset.habitQuick);
    const date = button.dataset.habitDate || h?.today, day = h?.days?.find(d => d.date === date);
    const payload = habitQuickPayload(h, day), key = `${owner}:${h?.id}:${date}`;
    if (!owner || !payload || quickSaving.has(key)) return;
    quickSaving.add(key);button.disabled = true;
    const restorePosition = rememberPosition(button);
    try {
      if ((await ctx.outbox().pendingHabits(owner)).some(item => item.habit === h.id && item.date === date)) {
        if (owner === state.me?.id) toast('Отметка уже ждёт отправки. Её состояние видно в очереди.');
        return;
      }
  ...
  async function openDay(id, date) {
    syncAccount();const owner = account, seq = ++request, known = findHabit(id);
    if (!known) return;
    date ||= known.today;
    show(`${header(known.title)}<p role="status">Открываем день…</p>`);
    try {
      const data = await api(`/api/personal/habits/${id}/tracker?from=${date}&to=${date}`);
      if (owner !== state.me?.id || seq !== request || !dialog().open) return;
      const day = data.days.find(d => d.date === date);
      if (!day) { await open(id, date);return; }
      show(`${header(known.title)}<div data-day-editor></div><button type="button" class="text-button" data-full-tracker>Календарь и статистика</button>`);
      dayEditor(data.habit, day, () => openDay(id, date));
      content().querySelector('[data-full-tracker]').addEventListener('click', () => open(id, date));
    } catch (error) { if (owner === state.me?.id && seq === request) { const status = content().querySelector('[role="status"]');if (status) status.textContent = error.message; } }
  }
  ...
  function dayEditor(h, day, refresh) {
    ...
    const simple = isSimpleHabit(r), measured = c?.state === 'measured';
    const value = habitMeasuredValue(c);
    const stateValue = c?.state === 'snoozed' ? 'measured' : c?.state || 'measured';
    const noteField = `<details class="habit-note-details" ${c?.note ? 'open' : ''}><summary>Заметка · необязательно</summary><label>Заметка<textarea name="note" maxlength="1000" rows="2" placeholder="Можно оставить пустой">${e(c?.note || '')}</textarea></label>${simple && c ? '<button type="submit" class="secondary" data-save-note>Сохранить заметку</button>' : ''}</details>`;
    const simpleActions = `<input type="hidden" name="state" value="${e(stateValue)}">${r.mode === 'build' ? `<input type="hidden" name="value" value="${value}">` : ''}<div class="habit-day-choices"><button type="button" class="primary" data-day-choice="success" aria-pressed="${day.state === 'success'}">${r.mode === 'quit' ? 'День без действия' : 'Выполнено'}</button><button type="button" class="secondary" data-day-choice="failed" aria-pressed="${day.state === 'failed'}">${r.mode === 'quit' ? 'Был срыв' : 'Не выполнено'}</button></div>${noteField}<details class="habit-extra-actions"><summary>Другие действия</summary>${r.mode === 'quit' ? `<label>Количество случаев, ${e(r.unit)}<input type="number" inputmode="decimal" name="value" min="0" max="1000000" step="any" value="${value}"></label><button type="button" class="secondary" data-save-count>Сохранить количество</button><p class="personal-muted">Укажите число, только если хотите учитывать количество. Ноль означает день без действия.</p>` : ''}<button type="button" class="text-button" data-day-choice="skipped">Пропустить день</button>${c ? '<button type="button" class="danger-text" data-clear>Убрать отметку</button>' : ''}</details>`;
    const measuredActions = `<input type="hidden" name="state" value="${e(stateValue)}"><label class="habit-measure-input">${e(habitMeasureLabel(r))}<input type="number" inputmode="decimal" name="value" min="0" max="1000000" step="any" value="${value}" placeholder="Например, ${r.mode === 'duration' ? '15' : '1,5'}" required></label><p class="personal-muted">${r.mode === 'reduce' ? 'Ноль тоже можно сохранить. День успешен, если значение не выше лимита.' : 'Можно указать часть цели. Несколько подходов складываются в общий итог дня.'}</p><div class="habit-increments">${(r.unit === 'мин' ? [5, 15, 30] : [1, 2, 5]).map(n => `<button type="button" class="text-button" data-increment="${n}">+${n} ${e(r.unit)}</button>`).join('')}<span class="personal-muted">Добавить к числу выше</span></div>${noteField}<div class="form-actions"><button type="submit" class="primary">${r.mode === 'duration' ? 'Сохранить время' : 'Сохранить количество'}</button></div><details class="habit-extra-actions"><summary>Другие действия</summary><button type="button" class="text-button" data-day-choice="failed">Не выполнено</button><button type="button" class="text-button" data-day-choice="skipped">Пропустить день</button>${c ? '<button type="button" class="danger-text" data-clear>Убрать отметку</button>' : ''}</details>`;
    root.innerHTML = `<form class="habit-day-editor"><h3>${e(dateLabel(day.date))}</h3><p class="habit-day-state" role="status">${e(habitStateLabel(day))}${simple && r.mode === 'quit' && measured && c.value > 0 ? ` · ${c.value} ${e(r.unit)}` : ''}</p>${simple ? '<p class="personal-muted">Нажатие сразу сохраняет отметку. Писать пояснение не нужно.</p>' : `<p>${e(goalLabel(r))}</p>`}${day.sourceDate ? `<p class="personal-muted">Перенесено с ${e(dateLabel(day.sourceDate))}</p>` : ''}${simple ? simpleActions : measuredActions}<p class="form-error" role="alert" hidden></p></form>${moveAllowed ? moveForm(day) : ''}`;
    const canPostpone=day.date===h.today&&!h.paused&&['pending','partial','snoozed'].includes(day.state);
    if(canPostpone){
      root.insertAdjacentHTML('beforeend',`<div class="habit-snooze-action"><p class="personal-muted">${e(habitSnoozeLabel(c,h.today,h.timezone))}</p><button type="button" class="text-button" data-postpone>Позже сегодня · 1 час</button><small class="personal-muted">Сохранённая отметка и заметка останутся. Напоминание придёт, если оно включено, с учётом времени привычки и тихих часов.</small></div>`);
    }
    const form = root.querySelector('form');bindDraft(form, `habit-day:${state.me.id}:${h.id}:${day.date}`);
    if (!['measured', 'failed', 'skipped'].includes(form.elements.state.value)) form.elements.state.value = 'measured';
    if (form.elements.note.value) form.querySelector('.habit-note-details').open = true;
    ...
    let busy = false;
    const save = async (deleting, choice = '') => {
      if (busy) return;
      if (choice) {
        form.elements.state.value = choice === 'success' || choice === 'count' ? 'measured' : choice;
        if (choice === 'success') form.elements.value.value = r.mode === 'quit' ? 0 : 1;
      } else if (!simple) form.elements.state.value = 'measured';
      const numeric = form.elements.state.value === 'measured';
      form.elements.value.required = numeric && (!simple || r.mode === 'quit');
      form.elements.value.disabled = !numeric;
      if (!deleting && !form.reportValidity()) { form.elements.value.disabled = false;return; }
      const owner = state.me?.id, seq = request;
      if (!owner) return;
      busy = true;form.inert = true;
      try {
        if ((await ctx.outbox().pendingHabits(owner)).some(item => item.habit === h.id && item.date === day.date)) throw new Error('Эта отметка уже ждёт отправки. Проверьте очередь перед изменением.');
        if (owner !== state.me?.id || seq !== request || !form.isConnected) return;
        const payload = { value: numeric ? Number(form.elements.value.value) : 0, state: form.elements.state.value, note: form.elements.note.value, expectedUpdatedAt: c?.updatedAt || '', revision: h.revision };
        if (deleting) await api(`/api/personal/habits/${h.id}/checkins/${day.date}`, { method: 'DELETE', headers: { 'X-Outbox-Owner': String(owner) }, body: JSON.stringify(payload) });
        else if (await saveMeasurement(h, day.date, payload)) {
          if (owner !== state.me?.id || seq !== request || !form.isConnected) return;
          clearDraft(form);form.innerHTML = '<p role="status">Отметка сохранена на этом устройстве. Календарь изменится после отправки на сервер.</p><button type="button" class="secondary" data-show-queue>Открыть очередь отправки</button>';
        ...
        if (owner !== state.me?.id || seq !== request || !form.isConnected) return;
        clearDraft(form);await loadPersonal({ force: true });
        if (owner === state.me?.id && seq === request && form.isConnected) { await refresh();toast(deleting ? 'Отметка убрана. Этот день снова без отметки.' : 'Отметка сохранена'); }
      }
      catch (error) { if (owner === state.me?.id && seq === request && form.isConnected) { const alert = form.querySelector('[role="alert"]');alert.textContent = error.message;alert.hidden = false; } }
      finally { busy = false;form.inert = false;if (form.elements.value) form.elements.value.disabled = false; }
    };
    form.addEventListener('submit', event => { event.preventDefault();void save(false); });
    form.querySelectorAll('[data-day-choice]').forEach(b => b.addEventListener('click', () => save(false, b.dataset.dayChoice)));
    form.querySelector('[data-save-count]')?.addEventListener('click', () => save(false, 'count'));
    ...
    root.querySelector('[data-postpone]')?.addEventListener('click',async()=>{
      ...
      try{
        ...
        await loadPersonal({force:true});await refresh();toast('Отложено. Сохранённая отметка и черновик остались на месте.');
  ...
  function settings(h = null) {
    ...
    const sync = () => {
      ...
      form.querySelectorAll('[data-mode-numeric]').forEach(label => { label.hidden = isSimpleHabit({ mode }); });
      ...
      form.querySelector('[data-mode-help]').textContent = mode === 'quit' ? 'Одна кнопка — день без нежелательного действия. Срыв отмечается отдельно. Объяснение или число случаев не обязательны.' : mode === 'build' ? 'Одно нажатие — выполнено. Без чисел и обязательных заметок. Отметку можно изменить или убрать.' : mode === 'reduce' ? 'Записывайте фактическое количество, включая ноль. Успех — значение не выше лимита.' : mode === 'duration' ? 'Время хранится в минутах. Можно добавлять несколько подходов к итогу дня.' : 'Для количественной цели можно записывать дробный и частичный результат.';
    };
  ...
  return { renderRow, renderList, bind, open, openDay, settings, confirmed };
}

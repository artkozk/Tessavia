...
[[ "$RELEASE_NAME" =~ ^20260907-calendar-work-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-calendar-work
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-calendar-work-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
release_created=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-calendar-work-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/tessavie-calendar-work-check.XXXXXX)
...
python3 /tmp/verify-calendar-preservation.py "$backup/business-control.db" "$dry/check.db"

check_http() {
  local base="$1"
  curl --max-time 15 -fsS "$base/" > "$dry/index.html"
  grep -qF 'app.js?v=20260907-calendar-work-1' "$dry/index.html"
  grep -qF 'styles.css?v=20260907-calendar-work-1' "$dry/index.html"
  curl --max-time 15 -fsS "$base/app.js?v=20260907-calendar-work-1" > "$dry/app.js"
  grep -qF 'chat-workspace.js?v=20260907-calendar-work-1' "$dry/app.js"
  grep -qF 'data-calendar-include-work' "$dry/app.js"
  curl --max-time 15 -fsS "$base/personal-calendar.js?v=20260907-calendar-work-1" > "$dry/calendar.js"
  grep -qF 'createPersonalCalendarUI' "$dry/calendar.js"
  grep -qF 'data-planner-journal' "$dry/app.js"
  curl --max-time 15 -fsS "$base/chat-workspace.js?v=20260907-calendar-work-1" > "$dry/chat.js"
  grep -qF 'createChatWorkspaceUI' "$dry/chat.js"
  curl --max-time 15 -fsS "$base/personal-today.js?v=20260907-calendar-work-1" > "$dry/today.js"
  grep -qF 'todayHiddenBlocks' "$dry/today.js"
  grep -qF 'today-empty-block' "$dry/app.js"
  curl --max-time 15 -fsS "$base/styles.css?v=20260907-calendar-work-1" > "$dry/styles.css"
  grep -qF -- '--chat-menu-left' "$dry/styles.css"
  curl --max-time 15 -fsS "$base/sw.js" > "$dry/sw.js"
  grep -qF 'tessavie-shell-20260907-calendar-work-1' "$dry/sw.js"
  ...
  test "$(curl --max-time 10 -s -o /dev/null -w '%{http_code}' "$base/api/personal/calendar?from=2026-09-01&to=2026-10-01")" = 401
}
...
test "$(sqlite3 "$db" "SELECT COUNT(*) FROM schema_migrations WHERE version='057_personal_calendar_work.sql';")" = 1

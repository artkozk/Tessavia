...
SCRIPT = ROOT / 'deploy/verify-mobile-gestures-today-database.py'
SPEC = importlib.util.spec_from_file_location('mobile_gestures_today_verifier', SCRIPT)
...
class UnchangedDatabaseTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fixtures = tempfile.TemporaryDirectory(prefix='tessavia-mobile-gestures-today-verifier-')
    ...
    def test_known_schema_matches_reviewed_migrations(self):
        actual = tuple(p.name for p in sorted((ROOT / 'internal/app/migrations').glob('*.sql')) if p.name[:3] <= '071')
    ...
    def test_unknown_receipt_blocks_rollback(self):
        self.execute("INSERT INTO schema_migrations VALUES('072_unknown.sql','2026-09-14')")
        with self.assertRaises(ValueError):
            self.rollback()
    ...
    def test_schema_070_is_refused_even_when_old_rows_are_valid(self):
        self.execute('DROP TRIGGER push_notification_created')
        self.execute('DROP TABLE push_deliveries')
        self.execute('DROP TABLE push_subscriptions')
        self.execute("DELETE FROM schema_migrations WHERE version='071_web_push.sql'")
        for check in (self.validate, self.rollback, self.verify):
            with self.assertRaises(ValueError):
                check()

    def test_cli_accepts_only_unchanged_071_and_never_exposes_push_credentials(self):
        self.seed_push()
        shutil.copyfile(self.after, self.before)
        result = subprocess.run([sys.executable, str(SCRIPT), str(self.before), str(self.after)], capture_output=True, text=True)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stdout.strip(), 'UNCHANGED_TABLES=129\nEXACT_SCHEMA_071=ok\nALL_DATA_UNCHANGED=ok\nNO_MIGRATIONS=ok')
        self.execute("UPDATE push_subscriptions SET auth='PRIVATE_changed_auth'")
        result = subprocess.run([sys.executable, str(SCRIPT), str(self.before), str(self.after)], capture_output=True, text=True)
        self.assertEqual(result.returncode, 2)
        self.assertNotIn('PRIVATE_', result.stdout + result.stderr)
        self.assertNotIn(str(self.after), result.stdout + result.stderr)

    def test_migration_mode_is_not_exposed_for_a_ui_only_release(self):
        original = self.after.read_bytes()
        ...
        self.assertNotIn('MIGRATION_071=ok', result.stdout)
        self.assertEqual(original, self.after.read_bytes())

    def test_deploy_pins_baseline_and_preserves_exact_data_in_both_trials(self):
        script = (ROOT / 'deploy/deploy-mobile-gestures-today-20260914.sh').read_text(encoding='utf-8')
        self.assertEqual(VERIFIER.ROLLBACK_BASELINE_COMMIT, 'f1aa7261962932d876bd1784d12f419b5fb542df')
        self.assertIn('test "$EXPECTED_PREVIOUS" = /opt/business-control/releases/20260914-mobile-access-f1aa726', script)
        self.assertIn("'cb24563873df325f74c024e79b330e728ba0acb9433377508f6dd8dd5fe03609' \"$previous/business-control\" | sha256sum -c -", script)
        self.assertIn("grep -qF 'app.js?v=20260914-mobile-access-1' \"$dry/rollback-index.html\"", script)
        self.assertNotIn('--migration ', script)
        self.assertNotIn('MIGRATION_071_PREEXISTING', script)
        self.assertNotIn('web_push_sending_unavailable', script)
        self.assertNotIn('c0de3fc', script)
        self.assertEqual(script.count('BUSINESS_REMINDERS_ENABLED=false BUSINESS_PUSH_ENABLED=false'), 2)
        self.assertEqual(script.count('python3 /tmp/verify-mobile-gestures-today-database.py "$backup/business-control.db" "$dry/check.db"'), 2)
        ...
        comparison = script.index('python3 /tmp/verify-mobile-gestures-today-database.py "$dry/rollback-before.db" "$dry/check.db"')
        cutover = script.index('systemctl stop business-control', script.index('ROLLBACK_COMPATIBILITY=ok'))
        self.assertLess(snapshot, old_binary)
        self.assertLess(old_binary, comparison)
        self.assertLess(comparison, cutover)
        self.assertIn('python3 /tmp/verify-mobile-gestures-today-database.py --validate-only "$backup/business-control.db"', script)
        self.assertIn('python3 /tmp/verify-mobile-gestures-today-database.py --rollback-safe "$db"', script)
        self.assertIn('python3 /tmp/verify-mobile-gestures-today-database.py --rollback-safe "$backup/cutover.db"', script)

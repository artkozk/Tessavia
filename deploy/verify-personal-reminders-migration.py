"""Verify migration 048 without printing or modifying private production rows."""
...
new_tables = {'personal_plan_reminders','personal_plan_reminder_sources','personal_plan_reminder_events'}
...
assert after.execute("SELECT COUNT(*) FROM reminder_preferences WHERE personal_enabled<>1 OR personal_enabled IS NULL").fetchone()[0] == 0
...
assert current - previous == {'048_personal_plan_reminders.sql'} and not previous - current

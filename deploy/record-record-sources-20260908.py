...
marker='[release:record-sources-20260908]'
evidence=(marker+' Выпущен '+commit+'; '+release+'; SHA256 '+sha+'; backup '+backup+'. SC-76: блок списка реальных записей, выбор источника/полей, поиск и настраиваемая кнопка существующей формы создания. Набор переносит схемы досок/этапов/полей/вариантов/начальных значений в одной читающей транзакции, без исходных записей и личных отметок. Установка атомарно создаёт новые IDs и переназначает связи/начальные select, сохраняя исходные доски. Проверены чужой источник, отсутствие исходных данных, независимость копий и rollback повреждённой зависимости.240Node, полныйGo, vet. Локальный UI: ручная сборка реестра из SC-67, поиск по каналу, сохранение набора, независимая установка пустой доски, создание новой записи с начальными Почта/2/Нет, открытие карточки, переименование и reload;320px. Исходные две записи сохранены. Dry run сравнил116таблиц, схема/данные неизменны; HTTPS/assets/401/health/nginx/integrity/FK успешны. PAGE_RECORD_SOURCES_2026_09_08.md. Произвольные формы и действия, настройка всех встроенных элементов и полная читалка ещё не завершены; большие задачи остаются открытыми.')
try:
 receipt={'commit':commit,'release':release,'sha256':sha,'backup':backup,'checkedAt':stamp(now),'tablesUnchanged':116,'schema':'064_page_app_composition.sql','nodeTests':240,'assets':'20260908-record-sources-3','tasks':[]}
 for task_id in ['5bf22f7a33f2cee34ab73fd3a6d62329','26c54f79aa1c9110536670ae8371a868','66da4e412e714f4ea85c56ff85de03ec']:
  ...
  if task_id=='5bf22f7a33f2cee34ab73fd3a6d62329' and detail['record']['status']!='completed':
  ...
  if task_id=='5bf22f7a33f2cee34ab73fd3a6d62329':assert updated['record']['status']=='completed'
  receipt['tasks'].append({'id':task_id,'status':updated['record']['status']})
 Path('/tmp/tessavie-record-sources-receipt.json').write_text(json.dumps(receipt,ensure_ascii=False,indent=2));print(json.dumps(receipt,ensure_ascii=False))
finally:

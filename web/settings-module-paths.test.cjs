...
test('own page keeps editing and sharing callable without displaying configuration buttons in viewing mode',async()=>{
 ...
 const ctx=vm.createContext({createPageSheetUI:()=>({mount(){},reset(){}}),createPageFinanceUI:()=>({mount(){}}),updatePageFinanceConfig:()=>false,createPageMediaUI:()=>({mount(){}}),createPageDataUI:()=>({mount(){}}),applyElementStyles(){},mountPageForms(){},recordActionMenu(){},recordRowText(){},Intl});

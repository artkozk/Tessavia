...
[[ "$RELEASE_NAME" =~ ^20260903-collection-heading-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-collection-heading
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-collection-heading-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''; dry_pid=''; switch_started=0
cleanup() {
  result=$?
  trap - EXIT
  if [[ -n "$dry_pid" ]]; then kill "$dry_pid" 2>/dev/null || true; wait "$dry_pid" 2>/dev/null || true; fi
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-collection-heading-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-collection-heading-check.XXXXXX)
...
check_http() {
  ...
  grep -q '20260903-collection-heading-1' "$dry/index.html"

...
ASSETS = '20260914-component-recovery-1'
MARKER = '[release:component-recovery-20260914]'
ROOT_ID = '38bf4aee34d6faf2e31c1e99d848ec99'
SPECS = ({'id': '2833a2fd2705b2e1785892848be48ff1', 'requestMarker': '[request:component-recovery-20260914]', 'title': 'P1 · Сохранять запрос библиотеки блоков до подтверждённого результата', 'result': 'Запрос создания или вставки блока отправляется только после сохранения неизменяемого повтора. После reload неизвестный результат открывается до редактирования; удалить этот черновик нельзя. Повторные 401/403/404 сохраняют его, успешная повторная проверка не создаёт вторую копию. Обычные ошибки ввода возвращают исправляемую форму. Общее направление библиотеки и конструктора остаётся в работе.'},)
...
def main():
    ...
    match = re.fullmatch(r'/opt/business-control/releases/20260914-component-recovery-([a-f0-9]{7,40})', release)
    ...
    assert backup_path.name.startswith('pre-component-recovery-')
    ...
    assert (backup_path / 'previous-release.txt').read_text().strip() == '/opt/business-control/releases/20260914-block-capacity-a8245f0'
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            'release_database_verifier', Path(__file__).with_name('verify-component-recovery-database.py'))
        ...
        for detail in parents.values():
            ...
            assert detail['record']['status'] == 'in_progress', 'Library parent needs fresh review'
        ...
        common = (
            ...
            'только бинарник и сохраняет последнюю базу; предыдущий a8245f0 поддерживает активную корзину и резервирование ID. Сохраняются новые отметки '
            ...
            'Документы: docs/operations/COMPONENT_RECOVERY_2026_09_14.md; '
            'docs/operations/COMPONENT_RECOVERY_RELEASE_SAFETY_2026_09_14.md. '
        ...
        Path('/tmp/component-recovery-receipt.json').write_text(

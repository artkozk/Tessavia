...
[[ "$RELEASE_NAME" =~ ^20260904-habit-tracker-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-habit-tracker
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-habit-tracker-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-habit-tracker-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-habit-tracker-check.XXXXXX)
...
python3 /tmp/verify-habit-migration.py "$backup/business-control.db" "$dry/check.db"

check_http() {
  local base="$1"
  curl --max-time 15 -fsS "$base/habit-tracker.js?v=20260904-habits-3" > "$dry/habit-tracker.js"
  ...
  grep -q '20260904-habits-3' "$dry/index.html"
  ...
  grep -qF "20260904-habits-3" "$dry/sw.js"

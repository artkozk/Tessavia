...
func (s *Server) handleExportProject(w http.ResponseWriter, r *http.Request) {
	...
	tables := map[string]exportQuery{
		...
		"workspaceNavigation":    {`SELECT * FROM workspace_navigation WHERE workspace_id = ?`, workspaceArg},
		"workspacePages":         {`SELECT * FROM workspace_pages WHERE workspace_id = ?`, workspaceArg},
		...
		"myGraphLayouts":         {`SELECT * FROM user_graph_layouts WHERE workspace_id=? AND user_id=?`, []any{workspaceID, currentUser(r).ID}},
	...
	payload := map[string]any{"schemaVersion": 17, "workspaceId": workspaceID, "exportedAt": nowText(), "exportedBy": currentUser(r).Username, "tables": map[string]any{}}

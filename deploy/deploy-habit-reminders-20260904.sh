...
[[ "$RELEASE_NAME" =~ ^20260904-habit-reminders-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-habit-reminders
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-habit-reminders-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-habit-reminders-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-habit-reminders-check.XXXXXX)
...
python3 /tmp/verify-habit-reminders-migration.py "$backup/business-control.db" "$dry/check.db"
...
check_http() {
  ...
  curl --max-time 15 -fsS "$base/habit-tracker.js?v=20260904-habit-reminders-1" > "$dry/habit-tracker.js"
  ...
  grep -q '20260904-habit-reminders-1' "$dry/index.html"
  ...
  grep -qF "20260904-habit-reminders-1" "$dry/sw.js"

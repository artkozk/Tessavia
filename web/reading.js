...
export function createReadingUI(ctx) {
  ...
  function paint() {
    ...
    root().innerHTML = `<div class="reading-page"><header class="reading-hero"><div><p class="eyebrow">Домашняя группа · ${e(displayDay(data.today, true))} · Москва</p><h2>Время для Слова</h2><p>Выберите книгу и одним нажатием отметьте прочитанную главу.</p></div><button class="secondary reading-main-action" data-reading-log>${icon('bookOpen')} Диапазон / частично</button></header>
      <nav class="reading-tabs" role="tablist" aria-label="Разделы чтения">${tabs.map(([id, title]) => `<button class="reading-tab" role="tab" data-reading-tab="${id}" aria-selected="${tab === id}" aria-current="${tab === id ? 'page' : 'false'}">${title}</button>`).join('')}</nav>
      ${!data.groupId ? `<section class="reading-panel reading-join-panel"><div><p class="eyebrow">Первый шаг</p><h3>Выберите домашнюю группу</h3><p>Ваши отметки останутся личными, а в общий рейтинг попадёт только количество прочитанных глав.</p></div><form id="reading-join"><label>Группа<select name="groupId">${groupOptions('')}</select></label><button class="primary">Присоединиться</button></form></section>` : ''}
    ...
    if (tabStrip && currentTab && tabStrip.scrollWidth > tabStrip.clientWidth) {
      tabStrip.scrollLeft = Math.max(0, currentTab.offsetLeft - (tabStrip.clientWidth - currentTab.offsetWidth) / 2);
    }
  ...
  function bookPicker() {
    body().innerHTML = `<section class="reading-book-picker"><header><div><p class="eyebrow">Книги Библии</p><h2>Выберите книгу</h2></div><button type="button" data-close aria-label="Закрыть">×</button></header><label class="reading-book-search"><span>Найти книгу</span><input type="search" inputmode="search" autocomplete="off" placeholder="Например, Иоанна"></label><div class="reading-legend">${[['never','Не читал'],['recent','До 7 дней'],['month','8–30 дней'],['old','31–90 дней'],['distant','Больше 90 дней']].map(([key,label])=>`<span class="reading-age-${key}">${label}</span>`).join('')}</div><h3>Ветхий Завет</h3>${bookGrid('old','data-reading-pick')}<h3>Новый Завет</h3>${bookGrid('new','data-reading-pick')}</section>`;
    ...
    body().querySelector('.reading-book-search input').addEventListener('input', event => {
      ...
      body().querySelectorAll('[data-reading-pick]').forEach(button => { button.hidden = query && !button.dataset.bookFilter.includes(query); });
    });
    openModal(dialog());
  }
  function readView() {
  ...
  function todayView() {
    ...
    return `<section class="reading-today-overview"><div class="reading-summary"><article><span>Сегодня</span><strong>${complete}</strong><small>${pluralNoun(complete, 'глава', 'главы', 'глав')}</small></article><article><span>Текущая серия</span><strong>${data.currentStreak}</strong><small>${pluralNoun(data.currentStreak, 'день', 'дня', 'дней')}</small></article><article><span>Лучшая серия</span><strong>${data.bestStreak}</strong><small>${pluralNoun(data.bestStreak, 'день', 'дня', 'дней')}</small></article></div>
      ...
      <div class="reading-columns"><section class="reading-panel reading-route-card"><p class="eyebrow">Личный маршрут</p><h3>${data.nextBook ? e(`${bookName(data.nextBook)} ${data.nextChapter}`) : 'Маршрут завершён'}</h3><p>Продолжите с главы после последнего полного личного чтения. План группы ведётся отдельно.</p>${data.nextBook ? `<button class="primary" data-reading-next>${icon('bookOpen')} Открыть следующую главу</button>` : `<button class="secondary" data-reading-log>${icon('bookOpen')} Выбрать новое чтение</button>`}</section><section class="reading-panel"><p class="eyebrow">Достижения</p><h3>Шаг за шагом</h3><div class="reading-badges">${[7, 30, 90, 365].map(n => `<span class="${data.bestStreak >= n ? 'earned' : ''}">${data.bestStreak >= n ? '✓ ' : ''}${n} дней</span>`).join('')}</div><p>Открытый рубеж остаётся в достижениях, даже если текущая серия прервалась.</p></section></div>
      ${plansView(true)}`;
  }

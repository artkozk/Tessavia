    ...
    try {
      ...
      form.onsubmit=async event=>{
        ...
        try {
          ...
          await read('/api/me/reminders',{method:'PUT',body:JSON.stringify({deadlineEnabled:form.elements.deadlineEnabled.checked,personalEnabled:form.elements.personalEnabled.checked,timezone:form.elements.timezone.value,quietStart:form.elements.quietStart.value,quietEnd:form.elements.quietEnd.value,projects,expectedUpdatedAt:form.elements.version.value})});
          ...
          if([403,409].includes(failure.status)){
            ...
            review.onclick=async()=>{
              ...
              try {
                ...
                error.textContent=`Сейчас сохранено: сроки карточек ${latest.deadlineEnabled?'включены':'выключены'}, личные дела ${latest.personalEnabled?'включены':'выключены'}; ${latest.timezone}; ${latest.quietStart?`${latest.quietStart}–${latest.quietEnd}`:'без тихих часов'}. Выключенные проекты: ${disabled.join(', ')||'нет'}. Ваши поля сохранены в форме.`;

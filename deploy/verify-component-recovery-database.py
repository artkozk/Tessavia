...
Startup must preserve every table value. The pinned rollback baseline a8245f0
...
ROLLBACK_BASELINE_COMMIT = 'a8245f0a866fda9a659a30e14722052a9f8f0868'
...
def rollback_safe(path):
    with closing(connect_readonly(path)) as db:
        # a8245f0 supports trash snapshots, receipts, restore and ID reservation.
...
if __name__ == '__main__':
    if len(sys.argv) == 3 and sys.argv[1] == '--rollback-safe':
        ...
        raise SystemExit('Usage: verify-component-recovery-database.py BEFORE AFTER | --validate-only DB | --rollback-safe DB')

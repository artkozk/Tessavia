...
[[ "$RELEASE_NAME" =~ ^20260907-templates-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-templates
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-templates-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
release_created=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-templates-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/tessavie-templates-check.XXXXXX)
...
check_http() {
  ...
  grep -qF 'app.js?v=20260907-templates-5' "$dry/index.html"
  grep -qF 'styles.css?v=20260907-templates-5' "$dry/index.html"
  curl --max-time 15 -fsS "$base/app.js?v=20260907-templates-5" > "$dry/app.js"
  grep -qF 'chat-workspace.js?v=20260907-templates-5' "$dry/app.js"
  grep -qF 'data-collection-picker' "$dry/app.js"
  grep -qF 'data-planner-journal' "$dry/app.js"
  curl --max-time 15 -fsS "$base/chat-workspace.js?v=20260907-templates-5" > "$dry/chat.js"
  grep -qF 'createChatWorkspaceUI' "$dry/chat.js"
  curl --max-time 15 -fsS "$base/personal-today.js?v=20260907-templates-5" > "$dry/today.js"
  grep -qF 'todayHiddenBlocks' "$dry/today.js"
  grep -qF 'today-empty-block' "$dry/app.js"
  curl --max-time 15 -fsS "$base/styles.css?v=20260907-templates-5" > "$dry/styles.css"
  grep -qF -- '--chat-menu-left' "$dry/styles.css"
  curl --max-time 15 -fsS "$base/sw.js" > "$dry/sw.js"
  grep -qF 'tessavie-shell-20260907-templates-5' "$dry/sw.js"

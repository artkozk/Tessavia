"""Verify personal conversation archive migration preserving existing data against an isolated same-server backup."""
import json
import sqlite3
import sys
...
assert tables(after)==tables(before)|{'chat_personal_archives'}
assert after.execute('SELECT count(*) FROM chat_personal_archives').fetchone()[0]==0
assert after.execute("SELECT count(*) FROM schema_migrations WHERE version='062_chat_personal_archives.sql'").fetchone()[0]==1
def rows(db,table,columns):
    selection=','.join('"'+column.replace('"','""')+'"' for column in columns)
    return sorted(json.dumps(dict(row),sort_keys=True,ensure_ascii=False) for row in db.execute('SELECT '+selection+' FROM "'+table.replace('"','""')+'"') if table!='schema_migrations' or row['version']!='062_chat_personal_archives.sql')

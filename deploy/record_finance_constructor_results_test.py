...
SPEC = importlib.util.spec_from_file_location('finance_constructor_recorder', Path(__file__).with_name('record-finance-constructor-results-20260915.py'))
REC = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(REC)


class FinanceConstructorRecorderChecks(unittest.TestCase):
    def setUp(self):
        ...
        self.acceptance = {'goTests': True, 'goVet': True, 'migrationPreserved': True, 'rollbackTrial': True,
            'goTestCount': 435, 'nodeTests': 664, 'buildInputs': 540, 'verifierTests': 13,
        ...
        self.identity = ('/opt/business-control/releases/20260915-finance-constructor-aaaaaaa', 'a' * 40,
                         'b' * 64, '/var/lib/business-control/backups/pre-finance-constructor-20260915T100000Z')
    ...
    def test_missing_checks_and_invented_physical_acceptance_are_rejected(self):
        ...
        for key, value in (('goTests', False), ('goTestCount', True), ('goTestCount', 434),
                           ('nodeTests', 663), ('verifierTests', 12), ('rollbackTrial', False),
    ...
    def test_exact_release_commit_and_backup_prefix_are_bound(self):
        ...
        for index, value in ((0, '/opt/business-control/releases/20260915-smart-select-aaaaaaa'),
                             (0, '/opt/business-control/releases/20260915-finance-constructor-aaaaaaa/../other'),
    ...
    def test_generated_evidence_uses_actual_widths_counts_and_exactly_the_two_ids(self):
        ...
        for text in evidence.values():
            ...
            self.assertIn('664 Node-тестов', text)
            self.assertIn('Браузерные ширины 320,390,1280 px', text)
            self.assertIn('075/143 таблиц', text)
    ...
    def test_prior_release_tasks_cannot_be_read_or_written(self):
        for task_id in ('e8f9f56433e9e5969e0abd2e3add2b12', 'de274e87c23aa7ce431d76f103851f4e'):
...
class FinanceConstructorReleaseChecks(unittest.TestCase):

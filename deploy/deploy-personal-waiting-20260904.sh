...
[[ "$RELEASE_NAME" =~ ^20260904-personal-waiting-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-personal-waiting
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-personal-waiting-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-personal-waiting-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-personal-waiting-check.XXXXXX)
...
python3 /tmp/verify-personal-waiting-migration.py "$backup/business-control.db" "$dry/check.db"
...
check_http() {
  ...
  grep -q '20260904-personal-waiting-4' "$dry/index.html"
  ...
  grep -qF "20260904-personal-waiting-4" "$dry/sw.js"

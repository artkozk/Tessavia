...
[[ "$RELEASE_NAME" =~ ^20260903-personal-planning-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-personal-planning
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-personal-planning-check.* ]]; then rm -rf -- "$dry"; fi
...
python3 - <<'PY'
import socket
with socket.socket() as s: s.bind(('127.0.0.1', 18625))
PY
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-personal-planning-check.XXXXXX)
cp "$backup/business-control.db" "$dry/check.db"
mkdir "$dry/uploads"
chown -R business-control:business-control "$dry"
runuser -u business-control -- env BUSINESS_ADDRESS=127.0.0.1:18625 BUSINESS_DATABASE_PATH="$dry/check.db" BUSINESS_UPLOAD_PATH="$dry/uploads" "$release/business-control" > "$dry/server.log" 2>&1 &
dry_pid=$!
for attempt in $(seq 1 30); do
  if curl --max-time 3 -fsS http://127.0.0.1:18625/api/health > "$dry/health.json" 2>/dev/null; then break; fi
...
expected_new = {'personal_projects','personal_goals','personal_recurrence_rules'}
assert new_tables == old_tables | expected_new, (new_tables-old_tables, old_tables-new_tables)
for table in expected_new:
    assert after.execute('SELECT COUNT(*) FROM "'+table+'"').fetchone()[0] == 0, table
expected_columns = {
    'item_kind':'task', 'project_id':None, 'goal_id':None, 'parent_id':None,
    'planned_minutes':0, 'actual_minutes':0, 'starts_at':None, 'ends_at':None,
    'series_id':'', 'occurrence_date':'', 'occurrence_state':'single',
}
count = after.execute('SELECT COUNT(*) FROM personal_plans').fetchone()[0]
for column, default in expected_columns.items():
    query = 'SELECT COUNT(*) FROM personal_plans WHERE "'+column+'" IS NOT ?'
    assert after.execute(query, (default,)).fetchone()[0] == 0, (column, count)
assert before.execute("SELECT COUNT(*) FROM schema_migrations WHERE version='035_personal_planning.sql'").fetchone()[0] == 0
...
check_http() {
  ...
  grep -q '20260903-personal-planning-1' "$dry/index.html"
  ...
  grep -qF "20260903-personal-planning-1" "$dry/sw.js"
...
check_http http://127.0.0.1:18625

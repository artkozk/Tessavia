"""Verify migration 049 without printing or modifying private production rows."""
...
new_tables = {'habit_reminder_preferences','habit_reminder_sources'}
...
assert after.execute("SELECT COUNT(*) FROM reminder_preferences WHERE habits_enabled<>1 OR habits_enabled IS NULL").fetchone()[0] == 0
...
assert current - previous == {'049_habit_reminder_delivery.sql'} and not previous - current

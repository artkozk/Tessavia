...
[[ "$RELEASE_NAME" =~ ^20260903-work-board-[a-f0-9]+$ ]]
...
python3 - <<'PY'
import socket
with socket.socket() as s: s.bind(('127.0.0.1', 18548))
PY
...
dry=$(mktemp -d /tmp/tessavie-work-board-check.XXXXXX)
cp "$backup/business-control.db" "$dry/check.db"
mkdir "$dry/uploads"
chown -R business-control:business-control "$dry"
runuser -u business-control -- env BUSINESS_ADDRESS=127.0.0.1:18548 BUSINESS_DATABASE_PATH="$dry/check.db" BUSINESS_UPLOAD_PATH="$dry/uploads" "$release/business-control" > "$dry/server.log" 2>&1 &
dry_pid=$!
for attempt in $(seq 1 30); do
  if curl --max-time 3 -fsS http://127.0.0.1:18548/api/health > "$dry/health.json" 2>/dev/null; then break; fi
...
check_http() {
  ...
  grep -q '20260903-work-board-1' "$dry/index.html"
  ...
  grep -qF ":scope > .idea-stage-board, :scope > .collection-board" "$dry/app.js"
  grep -qF "Доска и её настройки" "$dry/app.js"
  grep -qF "const workRecords = { ...list" "$dry/app.js"
  ...
  python3 -c 'import json,sys; m=json.load(open(sys.argv[1])); assert m["name"]=="Tessavie" and m["background_color"]=="#faf9f5" and m["theme_color"]=="#14725e" and all("classic-1" in i["src"] for i in m["icons"])' "$dry/manifest.json"
  for asset in tessavie-mark.svg tessavie-32.png tessavie-180.png tessavie-192.png tessavie-512.png tessavie-maskable-512.png; do
...
check_http http://127.0.0.1:18548
...
cmp /etc/business-control.env "$backup/business-control.env"

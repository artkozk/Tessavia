...
ASSETS = '20260914-block-capacity-1'
MARKER = '[release:block-capacity-20260914]'
ROOT_ID = '66da4e412e714f4ea85c56ff85de03ec'
SPECS = ({'id': '1908592abd805fc870faf8fbee62092f', 'requestMarker': '[request:block-capacity-20260914]', 'title': 'P1 · Освобождать место для блоков без тупика скрытых элементов', 'result': 'Блоки можно перенести в корзину с сохранением всей группы и прежних идентификаторов. Скрытие продолжает занимать место и обозначено отдельно. Восстановление возвращает отметки и числа, проверяет лимит и зависимости; неизвестный ответ повторяется тем же запросом. Общий конструктор остаётся в работе.'},)
...
def main():
    ...
    match = re.fullmatch(r'/opt/business-control/releases/20260914-block-capacity-([a-f0-9]{7,40})', release)
    ...
    assert backup_path.name.startswith('pre-block-capacity-')
    ...
    assert (backup_path / 'previous-release.txt').read_text().strip() == '/opt/business-control/releases/20260914-constructor-safety-ee7b320'
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            'release_database_verifier', Path(__file__).with_name('verify-block-capacity-database.py'))
        ...
        common = (
            ...
            'только бинарник и сохраняет последнюю базу; при незавершённых записях корзины откат блокируется из-за резервирования ID. Сохраняются новые отметки '
            ...
            'Документы: docs/operations/BLOCK_CAPACITY_2026_09_14.md; '
            'docs/operations/BLOCK_CAPACITY_RELEASE_SAFETY_2026_09_14.md. '
        ...
        receipt = {
            ...
            'rollbackCompatibility': 'verified_before_use_blocked_with_pending_trash_latest_data_retained',
        ...
        Path('/tmp/block-capacity-receipt.json').write_text(

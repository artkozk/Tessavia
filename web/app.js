import { selectMenuPosition, selectOptionScrollTop, trackSelectAnchor } from './select-positioning.js?v=20260915-team-accounting-1';
import { createMobileAccessUI, mobileLaunchURL } from './mobile-access.js?v=20260915-team-accounting-1';
import {personalPlanReferenceOptions,restorePersonalPlanReferenceChoice,personalPlanReferenceError,bindPersonalPlanReferenceFields,bindPersonalPlanDraftReview,personalPlanReferenceSummary,personalPlanReferenceDetails} from './personal-plan-references.js?v=20260915-team-accounting-1';
import {collectionFieldTypeNames, fieldConversionChoices, fieldConversionPreviewHTML, incompatibleChoiceDraft} from './field-conversion.js?v=20260915-team-accounting-1';
import { createPersonalFinanceUI } from './personal-finance.js?v=20260915-team-accounting-1';
import { createMediaVariantsUI } from './media-variants.js?v=20260915-team-accounting-1';
import { pageLabelTargets, applyPageLabels, updatePageTexts } from './page-labels.js?v=20260915-team-accounting-1';
import { createSettingsHub } from './settings-hub.js?v=20260915-team-accounting-1';
import { reviewFieldConflict } from './field-conflicts.js?v=20260915-team-accounting-1';
import { createPageAppUI } from './page-apps.js?v=20260915-team-accounting-1';
import { createChatGroupUI } from './chat-groups.js?v=20260915-team-accounting-1';
import { conversationFolder, filterConversations, conversationTimeLabel, pendingConversationItems, chatDraftKey, chooseConversation, readConversationDraft, writeConversationDraft, mergeChatHistory, createChatWorkspaceUI } from './chat-workspace.js?v=20260915-team-accounting-1';
import { createPersonalCalendarUI } from './personal-calendar.js?v=20260915-team-accounting-1';
import { createPersonalReviewUI } from './personal-review.js?v=20260904-personal-review-3';
import { createPersonalWaitingUI } from './personal-waiting.js?v=20260915-team-accounting-1';
import { createHabitReminderUI } from './habit-reminders.js?v=20260904-habit-reminders-1';
import { createPersonalRemindersUI } from './personal-reminders.js?v=20260915-team-accounting-1';
import { createReminderSettingsUI } from './reminder-settings.js?v=20260915-team-accounting-1';
import { personalRoute, personalNavigationItems, personalNavigationKey, personalNavigationTarget, navigationItemVisible, navigationOrderWithInactive } from './personal-navigation.js?v=20260915-team-accounting-1';
import { createPersonalTodayUI, useProgressiveToday } from './personal-today.js?v=20260915-team-accounting-1';
import { createFirstUseUI } from './first-use.js?v=20260915-team-accounting-1';
...
import { createLifeMapUI } from './life-map.js?v=20260915-team-accounting-1';
...
import { createNoteMediaUI, createNoteFileDraftStore } from './note-media.js?v=20260915-team-accounting-1';
import { createNoteLibraryUI, parseNoteTags } from './note-library.js?v=20260904-note-media-3';
import { createHabitUI, habitTodaySummary } from './habit-tracker.js?v=20260915-team-accounting-1';
import { installSwipeTabs, revealSwipeTab } from './swipe-tabs.js?v=20260915-team-accounting-1';
...
import { createOutboxUI } from './outbox-ui.js?v=20260915-team-accounting-1';
let offlineOutbox;
...
const state = {
  ...
  chatFilesOnly: false, chatSearch: '', chatSearchOpen: false, chatDigests: new Map(), chatDigestLoading: '', chatUploadItems: [], chatEditingMessageId: '',
...
offlineOutbox = createOutboxUI({
  ...
  onConfirmed: (item,result) => {
    ...
    if (['note','plan','habit-checkin','note-attachment','capture','note-to-plan'].includes(item.kind)) void loadPersonal({ force: true });
      ...
      if (!state.chatSearch && !state.chatFavoritesOnly && !state.chatFilesOnly && !state.chatHistoryAround && state.chatLoadedThreadId === item.thread) {
...
function bindGlobalEvents() {
  ...
  bindSidebarSwipe();
  installSwipeTabs({
    isEnabled: () => Boolean(state.me && !state.layoutDraft && !state.pageLayoutDraft && !$('.sidebar').classList.contains('open')),
    getContext: () => `${state.me?.id || ''}:${state.activeWorkspaceId || ''}:${state.view || ''}:${state.personalTab || ''}`,
  });
...
function workspaceHasActiveInput() {
  ...
  return Boolean(
    ...
    document.querySelector('.app-data-form[aria-busy="true"], .chat-message-menu[open], .chat-header-more[open], .chat-composer-more[open], .chat-emoji-picker, .chat-composer-context > div, .chat-main.drag-files, .chat-composer.is-recording')
  );
}
...
function navigationCatalog(preferences = state.interfacePreferences) {
  ...
  // Finance is available for personal opt-in even in an older team's module profile.
  // Availability in this catalogue does not add it to anybody's working sidebar.
  const builtin = personal ? personalNavigationItems() : navItems.filter(([key]) => key === 'personal' || key === 'finance' || enabled.has(key)).map(([key, label, iconName, group]) => ({ key, label, iconName, group, defaultVisible: key === 'personal' || enabled.has(key) }));
...
function bindPersonalInteractions() {
  ...
  habitUI.bind();
  $$('[data-swipe-tabs="personal-habits"]').forEach(revealSwipeTab);
...
function restoreChatDraft() {
  ...
  state.chatEditingMessageId=draft.edit?.id||'';state.chatEditDraft=draft.edit?{messageID:draft.edit.id,threadID:state.activeChatThreadId,context:captureProjectContext(),body:draft.edit.body}:null;state.chatHistoryQuery=null;state.chatHistoryAround='';state.chatSearch='';state.chatSearchOpen=false;state.chatFavoritesOnly=false;state.chatFilesOnly=false;state.chatPins=[];
}
function activateChatConversation(id) {
  state.chatFilesOnly=false;
  $('.chat-shell')?.classList.remove('show-threads');
  if(!persistChatDraft())return;
  chatListQuery();chatListSearch.archived=!!state.chatThreads.find(thread=>thread.id===id)?.archived;
...
async function jumpToChatMessage(id) {
  const fromFiles=state.chatFilesOnly;state.chatFilesOnly=false;
  const message=$(`#chat-message-${CSS.escape(id)}`);
  if(message&&!state.chatSearch&&!fromFiles){message.scrollIntoView({behavior:'smooth',block:'center'});message.classList.add('highlight');return;}
...
async function refreshPendingChat() {
  ...
  try {
    ...
    const items = pendingConversationItems(all,state.chatMessages,context).filter(item=>!state.chatFilesOnly||item.kind==='attachment');
...
function decorateChatUI() {
	...
	if (!main) return;
  if(state.chatFilesOnly) {
    $('.chat-context-stack',main)?.insertAdjacentHTML('afterbegin',`<div class="chat-filter-notice"><span>${icon('fileText')} Файлы и медиа</span><button type="button" class="text-button" data-chat-clear-files>Все сообщения</button></div>`);
    $('[data-chat-clear-files]',main)?.addEventListener('click',()=>{state.chatFilesOnly=false;state.chatSearch='';state.chatSearchOpen=false;state.chatHistoryAround='';state.chatLoadedThreadId='';renderChat();});
    const empty=$('.chat-empty',main);
    if(empty){$('strong',empty).textContent=state.chatSearch?'Вложения не найдены':'Вложений пока нет';$('p',empty).textContent=state.chatSearch?'Попробуйте другое имя файла или уберите поиск.':'Здесь появятся файлы, изображения, голосовые и видео этого разговора.';}
  }
...
let chatListSearch={context:'',query:'',archived:false};
function chatListQuery() {
  const context=JSON.stringify([state.me?.id,state.activeWorkspaceId,state.projectContextEpoch||0]);
  if(chatListSearch.context!==context)chatListSearch={context,query:'',archived:false};
  return chatListSearch.query;
}
function applyChatListFilter() {
  const query=chatListQuery(), matching=new Set(filterConversations(conversationFolder(state.chatThreads,chatListSearch.archived),query,chatThreadTitle).map(item=>item.id));
  $$('[data-chat-thread]').forEach(button=>{button.hidden=!matching.has(button.dataset.chatThread);});
  const empty=$('[data-chat-list-empty]');if(empty){empty.hidden=matching.size>0;empty.textContent=query.trim()?'Диалог не найден. Попробуйте другое имя или название.':chatListSearch.archived?'Архив пуст. Разговоры можно убрать сюда через их меню.':'В этом списке пока нет разговоров. Откройте архив или начните новый.';}
  const archived=state.chatThreads.filter(thread=>thread.archived),unread=archived.reduce((sum,thread)=>sum+thread.unreadCount,0),folder=$('[data-chat-archive-folder]');
  if(folder){folder.hidden=!archived.length&&!chatListSearch.archived;folder.innerHTML=chatListSearch.archived?`${icon('chevronLeft')} Все диалоги`:`${icon('archive')} Архив <span>${archived.length}${unread?` · новых ${unread}`:''}</span>`;folder.setAttribute('aria-pressed',String(chatListSearch.archived));}
  const heading=$('.chat-thread-list h1');if(heading)heading.textContent=chatListSearch.archived?'Архив':'Сообщения';
  const subtitle=$('.chat-thread-list > header p');if(subtitle)subtitle.textContent=chatListSearch.archived?'Разговоры скрыты только у вас':'Диалоги, группы и работа проекта';
...
async function loadChatThread(threadID, silent = false, older = false, newer = false) {
  const context=captureProjectContext(),query=state.chatSearch||'',around=state.chatHistoryAround||'',favorites=!!state.chatFavoritesOnly,attachments=!!state.chatFilesOnly;
  const signature=`${threadID}:${query}:${favorites}:${around}:${attachments}`;
  if((older||newer)&&state.chatHistoryLoading)return;
  if(silent&&(state.chatHistoryLoading||state.chatInputComposing||state.chatRecording||document.querySelector('.app-data-form[aria-busy="true"], .chat-message-menu[open],.chat-header-more[open],.chat-composer-more[open]')||[...document.querySelectorAll('.chat-messages audio,.chat-messages video')].some(media=>!media.paused)))return;
  const request=state.chatLoadRequest=(state.chatLoadRequest||0)+1;
  const params=new URLSearchParams();if(query)params.set('q',query);if(favorites)params.set('favorites','true');if(attachments)params.set('attachments','true');
  ...
  try {
    ...
    if(!query&&!favorites&&!attachments&&!around&&atBottom&&document.visibilityState==='visible'&&threads.find(item=>item.id===threadID)?.unreadCount>0) {
...
function bindChatEvents() {
  ...
  $('[data-clear-chat-list-search]')?.addEventListener('click',()=>{chatListSearch.query='';listSearch.value='';applyChatListFilter();listSearch.focus();});
	...
	bindChatDrawerSwipe();
  $('[data-chat-archive-folder]')?.addEventListener('click',()=>{chatListSearch.archived=!chatListSearch.archived;chatListSearch.query='';$('.chat-list-search input').value='';applyChatListFilter();});
  $$('[data-chat-thread-archive]').forEach(button=>button.addEventListener('click',async()=>{
    const thread=state.chatThreads.find(item=>item.id===state.activeChatThreadId);if(!thread)return;
    const context=captureProjectContext(),archived=!thread.archived;button.disabled=true;
    try {
      await api(`/api/chat/threads/${thread.id}/personal-archive`,{method:'PUT',headers:{'X-Workspace-ID':context.workspace},body:JSON.stringify({archived})});
      ...
      toast(archived?'Разговор убран в ваш архив':'Разговор возвращён из архива');
    } catch(error){if(isProjectContextCurrent(context))toast(error.message,true);}
    finally{if(button.isConnected)button.disabled=false;}
  }));
  ...
  $('[data-chat-files]')?.addEventListener('click',()=>{state.chatFilesOnly=!state.chatFilesOnly;state.chatFavoritesOnly=false;state.chatSearch='';state.chatSearchOpen=false;state.chatHistoryAround='';state.chatLoadedThreadId='';renderChat();});
	$('[data-chat-favorites]')?.addEventListener('click', () => { state.chatFilesOnly=false; state.chatFavoritesOnly = !state.chatFavoritesOnly; state.chatLoadedThreadId = ''; renderChat(); });
    ...
    const payload = { body, replyToId: state.chatReplyToId, linkedRecordId: state.chatLinkedRecordId };
      ...
      if(state.chatFilesOnly && context.owner === state.me?.id && context.workspace === state.activeWorkspaceId && context.thread === state.activeChatThreadId){state.chatFilesOnly=false;state.chatSearch='';state.chatSearchOpen=false;state.chatLoadedThreadId='';}

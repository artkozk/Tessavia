...
[[ "$RELEASE_NAME" =~ ^20260903-writing-access-[a-f0-9]+$ ]]
...
python3 - <<'PY'
import socket
with socket.socket() as s: s.bind(('127.0.0.1', 18554))
PY
...
dry=$(mktemp -d /tmp/tessavie-writing-access-check.XXXXXX)
cp "$backup/business-control.db" "$dry/check.db"
mkdir "$dry/uploads"
chown -R business-control:business-control "$dry"
runuser -u business-control -- env BUSINESS_ADDRESS=127.0.0.1:18554 BUSINESS_DATABASE_PATH="$dry/check.db" BUSINESS_UPLOAD_PATH="$dry/uploads" "$release/business-control" > "$dry/server.log" 2>&1 &
dry_pid=$!
for attempt in $(seq 1 30); do
  if curl --max-time 3 -fsS http://127.0.0.1:18554/api/health > "$dry/health.json" 2>/dev/null; then break; fi
...
expected = {'027_team_lifecycle.sql', '028_team_activity.sql', '031_notebook_title_origin.sql'}
old_versions = {r[0] for r in before.execute('SELECT version FROM schema_migrations')}
new_versions = {r[0] for r in after.execute('SELECT version FROM schema_migrations')}
assert new_versions - old_versions == expected, 'Unexpected migrations'
for table in tables:
    if table == 'schema_migrations':
        assert all(dict(r) in [dict(n) for n in after.execute('SELECT * FROM schema_migrations')] for r in before.execute('SELECT * FROM schema_migrations'))
        continue
    columns = [r[1] for r in before.execute('PRAGMA table_info("'+table+'")')]
    query = 'SELECT '+','.join('"'+c+'"' for c in columns)+' FROM "'+table+'" ORDER BY rowid'
...
assert after_tables - set(tables) == {'team_activity'}, 'Unexpected tables'
assert after.execute('SELECT COUNT(*) FROM team_activity').fetchone()[0] == 0
assert after.execute('SELECT COUNT(*) FROM teams WHERE deleted_at IS NOT NULL').fetchone()[0] == 0
for table in ('personal_notes','personal_plans','records'):
    assert after.execute('SELECT COUNT(*) FROM '+table+' WHERE title_generated<>0').fetchone()[0] == 0
...
check_http() {
  ...
  grep -q '20260903-writing-access-1' "$dry/index.html"
  ...
  grep -qF 'data-notebook-title' "$dry/styles.css"
...
check_http http://127.0.0.1:18554
...
test "$(sqlite3 "$db" "SELECT COUNT(*) FROM schema_migrations WHERE version IN ('027_team_lifecycle.sql', '028_team_activity.sql', '031_notebook_title_origin.sql');")" = 3

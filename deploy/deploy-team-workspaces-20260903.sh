...
[[ "$RELEASE_NAME" =~ ^20260903-team-workspaces-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-team-workspaces
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-team-workspaces-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-team-workspaces-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-team-workspaces-check.XXXXXX)
...
python3 /tmp/verify-team-workspace-migration.py "$backup/business-control.db" "$dry/check.db"

check_http() {
  local base="$1"
  curl --max-time 15 -fsS "$base/" > "$dry/index.html"
  grep -q '<title>Tessavie</title>' "$dry/index.html"
  grep -q '20260903-team-workspaces-2' "$dry/index.html"
  ...
  grep -qF "20260903-team-workspaces-2" "$dry/sw.js"

...
ASSETS = '20260914-mobile-habits-1'
MARKER = '[release:mobile-habits-20260914]'
ROOT_ID = '66da4e412e714f4ea85c56ff85de03ec'
SPECS = (
    {
        'id': '92118eef746fd910cab273e2026dc101',
        'requestMarker': '[request:mobile-menu-20260914]',
        'title': 'P0 · Свайп меню и стабильный экран телефона',
        'result': (
            'Мобильное меню закрывается левым жестом с плавным смещением. '
            'Обычный тап сохраняет переход, вертикальная прокрутка списка '
            'не считается закрытием; короткий или отменённый жест возвращает '
            'меню без случайного нажатия. Размер жеста связан с реальной '
            'шириной панели. Открытие и закрытие сохраняют позицию страницы. '
            'Анимация учитывает reduced motion; телефонные полосы прокрутки '
            'скрыты визуально при сохранении самой прокрутки. '
            'Это исправление оболочки и меню, а не подтверждение полного '
            'отсутствия всех мобильных ошибок платформы.'
        ),
    },
    {
        'id': 'eb8eb14c857b866d39c593ba6dc566f4',
        'requestMarker': '[request:habit-checkin-20260914]',
        'title': 'P0 · Простая отметка привычек без обязательного факта',
        'result': (
            'Бинарная привычка и отказ от нежелательного действия отмечаются '
            'нажатием на день без обязательной заметки и ввода числа. '
            'Существующую отметку можно исправить или убрать. Для количества '
            'и длительности остаётся понятный числовой ввод, включая ноль; '
            'заметка необязательна. Неотмеченный день не объявляется успехом '
            'или срывом. Ограничения пауз, архива, будущего и конфликта '
            'сохраняются. Список сохраняет позицию при отметке. '
            'Используется прежний API и совместимый формат записей; '
            'массовое переписывание истории привычек не выполняется.'
        ),
    },
)
...
def main():
    ...
    match = re.fullmatch(r'/opt/business-control/releases/20260914-mobile-habits-([a-f0-9]{7,40})', release)
    ...
    assert backup_path.name.startswith('pre-mobile-habits-')
    ...
    assert (backup_path / 'previous-release.txt').read_text().strip() == '/opt/business-control/releases/20260914-finance-expenses-b26be9d'
    ...
    for base in ('http://127.0.0.1:8522', 'https://control.e-rd.ru'):
        ...
        with client.open(base + '/habit-tracker.js?v=' + ASSETS, timeout=20) as response:
            assert b'export function habitQuickPayload(habit, day)' in response.read()
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            'release_database_verifier', Path(__file__).with_name('verify-mobile-habits-database.py'))
        ...
        common = (
            ...
            'Документы: docs/operations/MOBILE_HABITS_2026_09_14.md; '
            'docs/operations/MOBILE_HABITS_RELEASE_SAFETY_2026_09_14.md. '
        ...
        Path('/tmp/mobile-habits-receipt.json').write_text(

...
[[ "$RELEASE_NAME" =~ ^20260904-habit-layout-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-habit-layout
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-habit-layout-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-habit-layout-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-habit-layout-check.XXXXXX)
...
check_http() {
  ...
  grep -q '20260904-habit-layout-2' "$dry/index.html"
  ...
  grep -qF "20260904-habit-layout-2" "$dry/sw.js"

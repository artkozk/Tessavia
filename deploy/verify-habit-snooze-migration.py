"""Verify migration 050 without printing or modifying private production rows."""
...
assert after.execute("SELECT COUNT(*) FROM personal_habit_checkins WHERE snoozed_at<>'' OR snoozed_from<>'' OR snoozed_at IS NULL OR snoozed_from IS NULL").fetchone()[0] == 0
...
assert current - previous == {'050_habit_snooze_metadata.sql'} and not previous - current

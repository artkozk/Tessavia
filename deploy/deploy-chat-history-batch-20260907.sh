...
[[ "$RELEASE_NAME" =~ ^20260907-chat-history-batch-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-chat-history-batch
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-chat-history-batch-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
release_created=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-chat-history-batch-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/tessavie-chat-history-batch-check.XXXXXX)
...
python3 /tmp/verify-chat-history-batch-migration.py "$backup/business-control.db" "$dry/check.db"

...
export function createPersonalTodayUI({state,api,escapeHTML:esc,icon,renderPersonal,renderPlanRow,formatMinutes,openPlan,openRecurrence,refreshPersonal,openProject,togglePlan,openDay,openModal,closeDialog,bindDraft,clearDraft,flushDrafts,toast}){
  ...
  let cached=null,request=null,generation=0,refreshKey='',settingsRequest=0;
  ...
  async function openSettings(){
    const owner=state.me?.id;if(!owner)return false;
    const dialog=q('#workspace-dialog'),content=q('#workspace-dialog-content');if(dialog.open)return false;
    const turn=++settingsRequest;
    content.innerHTML=`<div class="workspace-editor-shell today-settings-dialog" data-day-settings-loading><header><h2>Границы дня</h2><button type="button" class="icon-button" data-today-close aria-label="Закрыть">${icon('x')}</button></header><p role="status">Загружаем ваши настройки…</p></div>`;
    const loading=q('[data-day-settings-loading]',content);
    const loadingAlive=()=>owner===state.me?.id&&turn===settingsRequest&&dialog.open&&loading.isConnected&&q('[data-day-settings-loading]',content)===loading;
    q('[data-today-close]',content).onclick=()=>closeDialog(dialog);openModal(dialog);
    async function loadSettings(){
      const status=q('[role=status]',loading);status.textContent='Загружаем ваши настройки…';
      try{
        const value=await read(owner,`/api/personal/day?timezone=${encodeURIComponent(zone())}`);
        if(!loadingAlive())return false;
        showSettings(value);return true;
      }catch(error){
        if(!loadingAlive())return false;
        status.textContent=error.message;
        const retry=document.createElement('button');retry.type='button';retry.className='text-button';retry.textContent='Повторить';
        retry.onclick=()=>{retry.disabled=true;void loadSettings();};status.append(retry);return false;
      }
    }
    ...
    bindSave(form,dialog,owner,()=>read(owner,'/api/personal/day/settings',{method:'PUT',body:JSON.stringify({timezone:form.elements.timezone.value,start:form.elements.start.value,end:form.elements.end.value,expectedUpdatedAt:form.elements.version.value})}),alive,'settings',value.date);
    }
    return loadSettings();
  }
  ...
  function bind(){
    ...
    q('[data-today-focus]')?.addEventListener('click',openFocus);
  }
  ...
  return {renderBlocks,renderPlans,bind,invalidate,openSettings,hasNoProjectContext,hiddenBlocks:(data,waiting,reminders)=>todayHiddenBlocks(data,current(),waiting,reminders)};
}

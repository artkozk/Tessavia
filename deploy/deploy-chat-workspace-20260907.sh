...
[[ "$RELEASE_NAME" =~ ^20260907-chat-workspace-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-chat-workspace
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-chat-workspace-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
release_created=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-chat-workspace-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/tessavie-chat-workspace-check.XXXXXX)
...
python3 /tmp/verify-chat-workspace-migration.py "$backup/business-control.db" "$dry/check.db"

check_http() {
  local base="$1"
  curl --max-time 15 -fsS "$base/" > "$dry/index.html"
  grep -qF 'app.js?v=20260907-chat-workspace-9' "$dry/index.html"
  grep -qF 'styles.css?v=20260907-chat-workspace-9' "$dry/index.html"
  curl --max-time 15 -fsS "$base/app.js?v=20260907-chat-workspace-9" > "$dry/app.js"
  grep -qF 'chat-workspace.js?v=20260907-chat-workspace-9' "$dry/app.js"
  grep -qF 'data-chat-history-older' "$dry/app.js"
  grep -qF 'data-planner-journal' "$dry/app.js"
  curl --max-time 15 -fsS "$base/chat-workspace.js?v=20260907-chat-workspace-9" > "$dry/chat.js"
  grep -qF 'createChatWorkspaceUI' "$dry/chat.js"
  curl --max-time 15 -fsS "$base/styles.css?v=20260907-chat-workspace-9" > "$dry/styles.css"
  grep -qF -- '--chat-menu-left' "$dry/styles.css"
  curl --max-time 15 -fsS "$base/sw.js" > "$dry/sw.js"
  grep -qF 'tessavie-shell-20260907-chat-workspace-9' "$dry/sw.js"

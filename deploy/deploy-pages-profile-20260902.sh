...
[[ "$RELEASE_NAME" =~ ^20260902-pages-profile-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-pages-profile
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
backup="/var/lib/business-control/backups/pre-pages-profile-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''; dry_pid=''; switch_started=0
cleanup() {
  result=$?
  trap - EXIT
  if [[ -n "$dry_pid" ]]; then kill "$dry_pid" 2>/dev/null || true; wait "$dry_pid" 2>/dev/null || true; fi
  if [[ -n "$dry" && "$dry" == /tmp/bizflow-pages-profile-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/bizflow-pages-profile-check.XXXXXX)
...
grep -q '20260902-pages-profile-1' "$dry/index.html"
...
grep -q '20260902-pages-profile-1' "$backup/local-index.html"
grep -q '20260902-pages-profile-1' "$backup/public-index.html"

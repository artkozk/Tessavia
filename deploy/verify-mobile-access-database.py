"""Read-only release checks for the reviewed 070 -> 071 Web Push migration.

Only two new tables, their indexes and the reviewed notification trigger are
allowed. The 127 existing tables retain their values, except for the single new
migration receipt. A binary-only rollback to pinned c0de3fc keeps schema 071 and
all current rows; Web Push sending is unavailable until a compatible worker is
restored. No database restoration or destructive schema downgrade occurs here.
"""
...
ROLLBACK_BASELINE_COMMIT = 'c0de3fca17b28d10558c089e505e4bcba1791a3b'
NEW_MIGRATION = '071_web_push.sql'
NEW_SCHEMA_SHA256 = '580d39fef18913de6e1fa6d1e3eca4599681f186c5d5e0eafdee6bd73de8770d'
NEW_TABLE_COUNT = 129
NEW_TABLES = {'push_subscriptions', 'push_deliveries'}
NEW_TRIGGER = 'push_notification_created'
...
def schema_level(db):
    digest = schema_digest(schema_objects(db))
    if digest == EXPECTED_SCHEMA_SHA256:
        count, migrations, level = EXPECTED_TABLE_COUNT, EXPECTED_MIGRATIONS, '070'
    elif digest == NEW_SCHEMA_SHA256:
        count, migrations, level = NEW_TABLE_COUNT, EXPECTED_MIGRATIONS + (NEW_MIGRATION,), '071'
    else:
        raise ValueError('Unknown schema')
    require(len(table_names(db)) == count, 'Unexpected table count')
    require(tuple(row[0] for row in db.execute('SELECT version FROM schema_migrations ORDER BY version'))
            == migrations, 'Unknown migration receipts')
    return level
...
def old_receipt_rows(db):
    rows = db.execute('SELECT version,applied_at FROM schema_migrations WHERE version != ?', (NEW_MIGRATION,))
    return sorted(json.dumps([(type(value).__name__, value.hex() if isinstance(value, bytes) else value)
                              for value in row], ensure_ascii=False, separators=(',', ':')) for row in rows)


def verify(before_path, after_path, migration=False):
    with closing(connect_readonly(before_path)) as before, closing(connect_readonly(after_path)) as after:
        validate(before)
        validate(after)
        before_level, after_level = schema_level(before), schema_level(after)
        tables = table_names(before, include_internal=True)
        if migration:
            require(before_level == '070' and after_level == '071', 'Expected exactly migration 071')
            projected = [item for item in schema_objects(after)
                         if item[2] not in NEW_TABLES and item[1] != NEW_TRIGGER]
            require(schema_objects(before) == projected, 'Existing schema changed')
            require(table_names(after, include_internal=True) == tables | NEW_TABLES, 'Unexpected added tables')
            for table in NEW_TABLES:
                require(not table_rows(after, table), 'Push worker or subscription mutated migration trial')
            require(old_receipt_rows(before) == old_receipt_rows(after), 'Existing migration receipt changed')
            receipt = after.execute('SELECT applied_at FROM schema_migrations WHERE version=?', (NEW_MIGRATION,)).fetchone()
            require(receipt and isinstance(receipt[0], str) and receipt[0].strip(), 'Missing migration timestamp')
        else:
            require(before_level == after_level, 'Use strict migration verification for a schema transition')
        ...
        for table in sorted(tables):
            if migration and table == 'schema_migrations':
                continue
            require(table_rows(before, table) == table_rows(after, table), 'Persisted table changed')
    if migration:
        print('OLD_TABLES_PRESERVED=127\nUNCHANGED_OLD_MIGRATION_RECEIPTS=70\nNEW_TABLES=2\nNEW_PUSH_TABLES_EMPTY=ok\nEXACT_SCHEMA_071=ok\nMIGRATION_071=ok')
    else:
        count = EXPECTED_TABLE_COUNT if before_level == '070' else NEW_TABLE_COUNT
        print(f'UNCHANGED_TABLES={count}\nEXACT_SCHEMA_{before_level}=ok\nALL_DATA_UNCHANGED=ok\nNO_MIGRATIONS=ok')


def rollback_safe(path):
    with closing(connect_readonly(path)) as db:
        # c0de3fc supports block trash and archived personal references; it can
        # run with the additive 071 tables/trigger but has no Web Push worker.
        validate(db)
        level = schema_level(db)
    print(f'ROLLBACK_SAFE=schema_{level}_block_trash_supported_latest_data_retained')
...
if __name__ == '__main__':
    if len(sys.argv) == 3 and sys.argv[1] == '--rollback-safe':
        ...
        try:
            ...
            raise SystemExit(2)
    elif len(sys.argv) == 4 and sys.argv[1] == '--migration':
        try:
            verify(*sys.argv[2:], migration=True)
        except Exception:
        ...
        raise SystemExit('Usage: verify-mobile-access-database.py BEFORE AFTER | --migration BEFORE070 AFTER071 | --validate-only DB | --rollback-safe DB')

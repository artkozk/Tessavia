...
func (s *Server) handleCreateTeamProject(w http.ResponseWriter, r *http.Request) {
	teamID := strings.TrimSpace(r.PathValue("id"))
	actorRole, ok := s.requireTeamAdmin(w, r, teamID)
	if !ok {
		return
	}
	var input struct {
	...
	if input.Name == "" || len([]rune(input.Name)) > 100 || len([]rune(input.Description)) > 800 {
		writeError(w, http.StatusBadRequest, "Название проекта обязательно и не длиннее 100 символов")
		return
	}
	id, err := newID()
	if err != nil {
	...
	slugBase := strings.Trim(collectionKeyPattern.ReplaceAllString(strings.ToLower(input.Name), "-"), "-")
	if slugBase == "" {
		slugBase = "project"
	}
	now := nowText()
	...
	if err != nil {
		writeError(w, http.StatusInternalServerError, "Не удалось начать создание проекта")
		return
	}
	defer tx.Rollback()
	var ownerID int64
	...
	if _, err = tx.ExecContext(r.Context(), `INSERT INTO workspaces(id, name, slug, kind, owner_id, delete_policy, description, team_id, created_at, updated_at) VALUES(?, ?, ?, 'team', ?, 'archive_only', ?, ?, ?, ?)`, id, input.Name, fmt.Sprintf("%s-%s", slugBase, id[:8]), ownerID, input.Description, teamID, now, now); err != nil {
	...
	rows, err := tx.QueryContext(r.Context(), `SELECT user_id, role FROM team_members WHERE team_id = ? AND status = 'active'`, teamID)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&member.userID, &member.role); err != nil {
			rows.Close()
			writeError(w, http.StatusInternalServerError, "Не удалось прочитать участника")
			return
		}
	...
	for _, member := range members {
		if member.role == "owner" || member.role == "admin" || selected[member.userID] {
			projectRole := "member"
			...
			if _, err = tx.ExecContext(r.Context(), `INSERT INTO workspace_members(workspace_id, user_id, role, status, joined_at) VALUES(?, ?, ?, 'active', ?)`, id, member.userID, projectRole, now); err != nil {
				writeError(w, http.StatusInternalServerError, "Не удалось выдать доступ к проекту")
				return
			}
		}
	}
	if err = tx.Commit(); err != nil {
		writeError(w, http.StatusInternalServerError, "Не удалось завершить создание проекта")
		return
	}
	writeJSON(w, http.StatusCreated, Workspace{ID: id, Name: input.Name, Slug: fmt.Sprintf("%s-%s", slugBase, id[:8]), Kind: "team", Role: actorRole, DeletePolicy: "archive_only", Description: input.Description, TeamID: teamID})
}

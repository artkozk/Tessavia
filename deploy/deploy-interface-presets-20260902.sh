...
[[ "$RELEASE_NAME" =~ ^20260902-interface-presets-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-interface-presets
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
backup="/var/lib/business-control/backups/pre-interface-presets-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/bizflow-presets-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/bizflow-presets-check.XXXXXX)
...
grep -q '20260902-presets-1' "$dry/index.html"
test "$(curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:18540/api/interface/presets)" = 401
...
test "$(sqlite3 "$dry/check.db" "SELECT COUNT(*) FROM schema_migrations WHERE version='023_interface_presets.sql';")" = 1
kill "$dry_pid"
...
grep -q '20260902-presets-1' "$backup/local-index.html"
grep -q '20260902-presets-1' "$backup/public-index.html"
test "$(systemctl is-active business-control)" = active
test "$(sqlite3 "$db" 'PRAGMA integrity_check;')" = ok
test "$(sqlite3 "$db" 'SELECT COUNT(*) FROM pragma_foreign_key_check;')" = 0
test "$(sqlite3 "$db" "SELECT COUNT(*) FROM schema_migrations WHERE version='023_interface_presets.sql';")" = 1

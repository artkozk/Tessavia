...
[[ "$RELEASE_NAME" =~ ^20260908-action-conditions-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-action-conditions
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-action-conditions-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
release_created=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-action-conditions-check.* ]]; then rm -rf -- "$dry"; fi
...
with sqlite3.connect('file:'+sys.argv[1]+'?mode=ro',uri=True) as db:
    for table in ('page_app_definitions','page_app_templates'):
        for row in db.execute('SELECT definition_json FROM '+table):
            if any(action.get('condition') is not None for block in json.loads(row[0]).get('blocks',[]) for action in block.get('actions',[])):
                sys.exit(2)
PYROLLBACK
    then
    ...
    printf 'ROLLED_BACK=%s\n' "$previous" >&2
      ...
      printf 'ROLLBACK_BLOCKED=saved_conditions_require_compatible_binary\n' >&2
    fi
...
dry=$(mktemp -d /tmp/tessavie-action-conditions-check.XXXXXX)
...
python3 /tmp/verify-action-conditions-migration.py "$backup/business-control.db" "$dry/check.db"

check_http() {
  local base="$1"
  curl --max-time 15 -fsS "$base/" > "$dry/index.html"
  grep -qF 'app.js?v=20260908-action-conditions-3' "$dry/index.html"
  grep -qF 'styles.css?v=20260908-action-conditions-3' "$dry/index.html"
  curl --max-time 15 -fsS "$base/app.js?v=20260908-action-conditions-3" > "$dry/app.js"
  grep -qF 'chat-workspace.js?v=20260908-action-conditions-3' "$dry/app.js"
  ...
  grep -qF 'page-apps.js?v=20260908-action-conditions-3' "$dry/app.js"
  curl --max-time 15 -fsS "$base/page-apps.js?v=20260908-action-conditions-3" > "$dry/page-apps.js"
  ...
  curl --max-time 15 -fsS "$base/page-forms.js?v=20260908-action-conditions-3" > "$dry/page-forms.js"
  ...
  curl --max-time 15 -fsS "$base/page-record-actions.js?v=20260908-action-conditions-3" > "$dry/page-record-actions.js"
  ...
  curl --max-time 15 -fsS "$base/page-action-conditions.js?v=20260908-action-conditions-3" > "$dry/page-action-conditions.js"
  grep -qF 'conditionMatches' "$dry/page-action-conditions.js"
  grep -qF 'conditionConfig' "$dry/page-action-conditions.js"
  curl --max-time 15 -fsS "$base/field-conflicts.js?v=20260908-action-conditions-3" > "$dry/field-conflicts.js"
  ...
  curl --max-time 15 -fsS "$base/chat-workspace.js?v=20260908-action-conditions-3" > "$dry/chat.js"
  grep -qF 'createChatWorkspaceUI' "$dry/chat.js"
  curl --max-time 15 -fsS "$base/life-map.js?v=20260908-action-conditions-3" > "$dry/life.js"
  grep -qF 'life-map-overview' "$dry/life.js"
  curl --max-time 15 -fsS "$base/chat-groups.js?v=20260908-action-conditions-3" > "$dry/groups.js"
  grep -qF 'createChatGroupUI' "$dry/groups.js"
  curl --max-time 15 -fsS "$base/personal-calendar.js?v=20260908-action-conditions-3" > "$dry/calendar.js"
  grep -qF 'openRecurrence' "$dry/calendar.js"
  grep -qF 'bindPersonalRecurrenceScope' "$dry/app.js"
  curl --max-time 15 -fsS "$base/personal-today.js?v=20260908-action-conditions-3" > "$dry/today.js"
  ...
  curl --max-time 15 -fsS "$base/styles.css?v=20260908-action-conditions-3" > "$dry/styles.css"
  grep -qF -- '--chat-menu-left' "$dry/styles.css"
  curl --max-time 15 -fsS "$base/sw.js" > "$dry/sw.js"
  grep -qF 'tessavie-shell-20260908-action-conditions-3' "$dry/sw.js"

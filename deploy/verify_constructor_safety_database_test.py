...
SCRIPT = ROOT / 'deploy/verify-constructor-safety-database.py'
SPEC = importlib.util.spec_from_file_location('constructor_safety_verifier', SCRIPT)
...
class UnchangedDatabaseTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fixtures = tempfile.TemporaryDirectory(prefix='tessavia-constructor-safety-verifier-')

...
[[ "$RELEASE_NAME" =~ ^20260903-graph-layouts-[a-f0-9]+$ ]]
...
dry=$(mktemp -d /tmp/tessavie-graph-layouts-check.XXXXXX)
...
for table in tables:
    ...
    if table == 'schema_migrations':
        assert len([r for r in new if r['version']=='033_graph_layouts.sql']) == 1
        new = [r for r in new if r['version']!='033_graph_layouts.sql']
...
assert after_tables == set(tables) | {'user_graph_layouts'}, 'Unexpected tables'
assert after.execute('SELECT COUNT(*) FROM user_graph_layouts').fetchone()[0] == 0
...
check_http() {
  ...
  grep -q '20260903-graph-layouts-1' "$dry/index.html"

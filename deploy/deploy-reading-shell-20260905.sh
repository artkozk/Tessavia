...
[[ "$RELEASE_NAME" =~ ^20260905-reading-shell-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-reading-shell
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-reading-shell-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
release_created=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-reading-shell-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/tessavie-reading-shell-check.XXXXXX)
cp "$backup/business-control.db" "$dry/check.db"
mkdir "$dry/uploads"
chown -R business-control:business-control "$dry"
runuser -u business-control -- env BUSINESS_REMINDERS_ENABLED=false BUSINESS_ADDRESS=127.0.0.1:18629 BUSINESS_DATABASE_PATH="$dry/check.db" BUSINESS_UPLOAD_PATH="$dry/uploads" "$release/business-control" > "$dry/server.log" 2>&1 &
dry_pid=$!
for attempt in $(seq 1 30); do
  if curl --max-time 3 -fsS http://127.0.0.1:18629/api/health > "$dry/health.json" 2>/dev/null; then break; fi
  test "$attempt" -lt 30
  sleep 1
done
python3 /tmp/verify-unchanged-database.py "$backup/business-control.db" "$dry/check.db"

check_http() {
  local base="$1"
  curl --max-time 15 -fsS "$base/" > "$dry/index.html"
  grep -qF 'styles.css?v=20260905-reading-shell-5' "$dry/index.html"
  grep -qF 'app.js?v=20260905-reading-shell-5' "$dry/index.html"
  curl --max-time 15 -fsS "$base/app.js?v=20260905-reading-shell-5" > "$dry/app.js"
  grep -qF "from './reading.js?v=20260905-reading-shell-5'" "$dry/app.js"
  grep -qF "from './outbox-ui.js?v=20260905-reading-shell-5'" "$dry/app.js"
  curl --max-time 15 -fsS "$base/reading.js?v=20260905-reading-shell-5" > "$dry/reading.js"
  grep -qF 'reading-hero page-heading' "$dry/reading.js"
  grep -qF 'reading-tab segment' "$dry/reading.js"
  grep -qF 'reading-panel section-panel' "$dry/reading.js"
  grep -qF 'data-reading-picker-testament' "$dry/reading.js"
  grep -qF 'currentTab.offsetLeft - 12' "$dry/reading.js"
  curl --max-time 15 -fsS "$base/styles.css?v=20260905-reading-shell-5" > "$dry/styles.css"
  grep -qF '.reading-page { width: 100%; max-width: none; margin: 0;' "$dry/styles.css"
  grep -qF 'border-bottom: 1px solid var(--line); border-radius: 0; background: transparent;' "$dry/styles.css"
  grep -qF '.reading-chapter-circles' "$dry/styles.css"
  curl --max-time 15 -fsS "$base/outbox-ui.js?v=20260905-reading-shell-5" > "$dry/outbox-ui.js"
  grep -qF "toastAction('Интерфейс Tessavie обновлён', 'Обновить', reload)" "$dry/outbox-ui.js"
  curl --max-time 15 -fsS "$base/sw.js" > "$dry/sw.js"
  grep -qF "tessavie-shell-20260905-reading-shell-5" "$dry/sw.js"
  grep -qF "client.postMessage({ type: 'tessavie-shell-updated'" "$dry/sw.js"
...
check_http http://127.0.0.1:18629
kill "$dry_pid"

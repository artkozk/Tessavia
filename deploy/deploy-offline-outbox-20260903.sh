...
[[ "$RELEASE_NAME" =~ ^20260903-offline-outbox-[a-f0-9]+$ ]]
...
python3 - <<'PY'
import socket
with socket.socket() as s: s.bind(('127.0.0.1', 18604))
PY
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-offline-outbox-check.XXXXXX)
cp "$backup/business-control.db" "$dry/check.db"
mkdir "$dry/uploads"
chown -R business-control:business-control "$dry"
runuser -u business-control -- env BUSINESS_ADDRESS=127.0.0.1:18604 BUSINESS_DATABASE_PATH="$dry/check.db" BUSINESS_UPLOAD_PATH="$dry/uploads" "$release/business-control" > "$dry/server.log" 2>&1 &
dry_pid=$!
for attempt in $(seq 1 30); do
  if curl --max-time 3 -fsS http://127.0.0.1:18604/api/health > "$dry/health.json" 2>/dev/null; then break; fi
...
old_tables = {r[0] for r in before.execute("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'")}
after_tables = {r[0] for r in after.execute("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'")}
assert after_tables == old_tables | {'personal_create_requests'}, after_tables-old_tables
for table in old_tables:
    columns = [r[1] for r in before.execute('PRAGMA table_info("'+table.replace('"','""')+'")')]
    select = ','.join('"'+c.replace('"','""')+'"' for c in columns)
    query = 'SELECT '+select+' FROM "'+table.replace('"','""')+'" ORDER BY rowid'
    old, new = [tuple(r) for r in before.execute(query)], [tuple(r) for r in after.execute(query)]
    if table == 'schema_migrations':
        new = [r for r in new if r[0] != '034_offline_create_receipts.sql']
...
chat_columns = {r[1] for r in after.execute('PRAGMA table_info(chat_messages)')}
assert 'client_payload_hash' in chat_columns
...
check_http() {
  ...
  grep -q '20260903-offline-outbox-1' "$dry/index.html"
  ...
  grep -qF "from './outbox-ui.js?v=20260903-offline-outbox-1'" "$dry/app.js"
  curl --max-time 15 -fsS "$base/offline-outbox.js?v=20260903-offline-outbox-1" > "$dry/offline-outbox.js"
  curl --max-time 15 -fsS "$base/outbox-ui.js?v=20260903-offline-outbox-1" > "$dry/outbox-ui.js"
  curl --max-time 15 -fsS "$base/sw.js" > "$dry/sw.js"
  grep -qF 'createIndexedOutbox' "$dry/offline-outbox.js"
  grep -qF "'/api/me'" "$dry/sw.js" && exit 1 || true
...
check_http http://127.0.0.1:18604
...
test "$(sqlite3 "$db" "SELECT COUNT(*) FROM pragma_table_info('chat_messages') WHERE name='client_payload_hash';")" = 1

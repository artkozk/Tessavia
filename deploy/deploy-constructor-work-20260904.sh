...
[[ "$RELEASE_NAME" =~ ^20260904-constructor-work-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-constructor-work
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-constructor-work-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-constructor-work-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-constructor-work-check.XXXXXX)
...
check_http() {
  ...
  grep -q '20260904-constructor-work-3' "$dry/index.html"
  ...
  grep -qF "20260904-constructor-work-3" "$dry/sw.js"

...
python3 - <<'PY'
import socket
with socket.socket() as s: s.bind(('127.0.0.1', 18614))
PY
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-work-toolbar-check.XXXXXX)
cp "$backup/business-control.db" "$dry/check.db"
mkdir "$dry/uploads"
chown -R business-control:business-control "$dry"
runuser -u business-control -- env BUSINESS_ADDRESS=127.0.0.1:18614 BUSINESS_DATABASE_PATH="$dry/check.db" BUSINESS_UPLOAD_PATH="$dry/uploads" "$release/business-control" > "$dry/server.log" 2>&1 &
dry_pid=$!
for attempt in $(seq 1 30); do
  if curl --max-time 3 -fsS http://127.0.0.1:18614/api/health > "$dry/health.json" 2>/dev/null; then break; fi
...
check_http() {
  ...
  grep -q '20260903-work-toolbar-2' "$dry/index.html"
...
check_http http://127.0.0.1:18614

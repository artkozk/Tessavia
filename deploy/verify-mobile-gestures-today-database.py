"""Read-only exact-schema 071 checks for the UI-only gestures/today release.

The pinned f1aa726 already supports Web Push, block trash and archived personal
references. No migration, schema downgrade or old-database restoration is
allowed: all 129 tables, receipts and current user data remain intact.
"""
...
def verify(before_path, after_path):
    ...
    print('UNCHANGED_TABLES=129\nEXACT_SCHEMA_071=ok\nALL_DATA_UNCHANGED=ok\nNO_MIGRATIONS=ok')


def rollback_safe(path):
    with closing(connect_readonly(path)) as db:
        # f1aa726 understands the same schema, push rows and block trash. Keep
        # latest user data; rollback only restores the earlier mobile UI.
        validate(db)
    print('ROLLBACK_SAFE=schema_071_block_trash_supported_latest_data_retained')


def validate_only(path):
    with closing(connect_readonly(path)) as db:
        validate(db)
    print('EXACT_SCHEMA_071=ok\nBLOCK_TRASH_STATE=ok\nDATABASE_VALIDATION=ok')
...
if __name__ == '__main__':
    if len(sys.argv) == 3 and sys.argv[1] == '--rollback-safe':
        ...
        raise SystemExit('Usage: verify-mobile-gestures-today-database.py BEFORE AFTER | --validate-only DB | --rollback-safe DB')

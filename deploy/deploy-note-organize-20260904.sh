...
[[ "$RELEASE_NAME" =~ ^20260904-note-organize-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-note-organize
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-note-organize-$(date -u +%Y%m%dT%H%M%SZ)"
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-note-organize-check.* ]]; then rm -rf -- "$dry"; fi
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-note-organize-check.XXXXXX)
...
python3 /tmp/verify-note-migration.py "$backup/business-control.db" "$dry/check.db"

check_http() {
  local base="$1"
  curl --max-time 15 -fsS "$base/note-library.js?v=20260904-note-organize-2" > "$dry/note-library.js"
  ...
  grep -q '20260904-note-organize-2' "$dry/index.html"
  ...
  grep -qF "20260904-note-organize-2" "$dry/sw.js"

...
[[ "$RELEASE_NAME" =~ ^20260904-note-media-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-note-media
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-note-media-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''; dry_pid=''; switch_started=0; release_created=0; nginx_changed=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-note-media-check.* ]]; then rm -rf -- "$dry"; fi
  if [[ "$result" -ne 0 && "$nginx_changed" -eq 1 ]]; then
    cp -- "$backup/business-control.nginx.conf" "$nginx_config"
...
rm -f -- "$staged"
dry=$(mktemp -d /tmp/tessavie-note-media-check.XXXXXX)
...
python3 /tmp/verify-note-media-migration.py "$backup/business-control.db" "$dry/check.db"
...
check_http() {
  ...
  grep -q '20260904-note-media-3' "$dry/index.html"
  ...
  grep -qF "from './outbox-ui.js?v=20260904-note-media-3'" "$dry/app.js"
  curl --max-time 15 -fsS "$base/offline-outbox.js?v=20260903-offline-outbox-3" > "$dry/offline-outbox.js"
  curl --max-time 15 -fsS "$base/outbox-ui.js?v=20260904-note-media-3" > "$dry/outbox-ui.js"
  curl --max-time 15 -fsS "$base/sw.js" > "$dry/sw.js"
  grep -qF "20260904-note-media-3" "$dry/sw.js"
...
p=Path('/etc/nginx/sites-enabled/business-control')
s=p.read_text();assert 'server_name control.e-rd.ru;' in s
assert s.count('client_max_body_size 1m;')==1
p.write_text(s.replace('client_max_body_size 1m;', 'client_max_body_size 16m;'))
PY
nginx -t

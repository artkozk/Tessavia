...
SPEC = importlib.util.spec_from_file_location('team_accounting_deploy_test', ROOT / 'deploy/build-team-accounting-deploy-20260915.py')
D = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(D)


class TeamAccountingDeployChecks(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_pins_previous_binary_and_distinct_current_cache(self):
        ...
        self.assertIn("grep -qF 'app.js?v=20260915-media-variants-1' \"$dry/rollback-index.html\"", self.script)
        self.assertIn("grep -qF 'app.js?v=20260915-team-accounting-1' \"$dry/index.html\"", self.script)
    ...
    def test_inherited_http_asset_and_auth_checks_are_not_dropped(self):
        normalize = lambda script: re.sub(r'v=20260915-(?:media-variants|team-accounting)-1', 'v=VERSION', script)
        ...
        for path in ('/api/workspace/finance/receipts', '/api/workspace/finance/receipts/unknown'):
    ...
    def test_validator_modes_and_latest_data_trial_are_preserved(self):
        verifier = 'python3 /tmp/verify-team-accounting-database.py'
        ...
        self.assertIn("version='077_workspace_cashbook.sql'", self.script)
        self.assertIn('= 150', self.script)
        self.assertIn('simple_receipts_unavailable_latest_data_retained', self.script)
        self.assertNotIn('/tmp/verify-media-variants-database.py', self.script)
    ...
    def test_uploaded_verifier_dependencies_cover_transitive_imports(self):
        ...
        pending = ['verify-team-accounting-database.py']
        visited = set()
        while pending:
            name = pending.pop()
            if name in visited:
    ...
    def test_changed_migration_is_rejected_before_script_is_generated(self):
        ...
        with patch.object(Path, 'read_text', changed):
            with self.assertRaisesRegex(ValueError, 'Reviewed migration changed: 077_workspace_cashbook.sql'):
                D.generate()

...
[[ "$RELEASE_NAME" =~ ^20260903-personal-scores-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-personal-scores
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
test "$previous" = "$EXPECTED_PREVIOUS"
backup="/var/lib/business-control/backups/pre-personal-scores-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''; dry_pid=''; switch_started=0
cleanup() {
  result=$?
  trap - EXIT
  if [[ -n "$dry_pid" ]]; then kill "$dry_pid" 2>/dev/null || true; wait "$dry_pid" 2>/dev/null || true; fi
  if [[ -n "$dry" && "$dry" == /tmp/tessavie-personal-scores-check.* ]]; then rm -rf -- "$dry"; fi
  if [[ "$result" -ne 0 && "$switch_started" -eq 1 ]]; then
    printf 'POST_SWITCH_CHECK_FAILED: data retained; candidate remains selected; fix forward. Previous binary cannot safely write the new score schema.\n' >&2
...
dry=$(mktemp -d /tmp/tessavie-personal-scores-check.XXXXXX)
...
for table in tables:
    ...
    columns = [r['name'] for r in before.execute('PRAGMA table_info('+quoted+')')]
    projection = ','.join('"'+c.replace('"','""')+'"' for c in columns)
    query = 'SELECT '+projection+' FROM '+quoted+' ORDER BY rowid'
    old, new = [dict(r) for r in before.execute(query)], [dict(r) for r in after.execute(query)]
    if table == 'schema_migrations':
        added = [r for r in new if r['version']=='032_personal_criterion_scores.sql']
        assert len(added)==1
        new = [r for r in new if r['version']!='032_personal_criterion_scores.sql']
...
assert after_tables == set(tables) | {'criterion_decisions'}, 'Unexpected tables'
assert after.execute('SELECT COUNT(*) FROM criterion_decisions').fetchone()[0] == 0
assert after.execute('SELECT COUNT(*) FROM records WHERE criterion_weight <> 1').fetchone()[0] == 0
...
check_http() {
  ...
  grep -q '20260903-personal-scores-1' "$dry/index.html"
  ...
  grep -qF 'function criterionAggregate(' "$dry/app.js"
  grep -qF 'criterion-personal-form' "$dry/app.js"
...
test "$(sqlite3 "$db" "SELECT COUNT(*) FROM schema_migrations WHERE version='032_personal_criterion_scores.sql';")" = 1

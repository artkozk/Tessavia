...
[[ "$RELEASE_NAME" =~ ^20260902-all-pages-[a-f0-9]+$ ]]
...
staged=/tmp/business-control-all-pages
db=/var/lib/business-control/business-control.db
previous=$(readlink -f "$root/current")
backup="/var/lib/business-control/backups/pre-all-pages-$(date -u +%Y%m%dT%H%M%SZ)"
dry=''
dry_pid=''
switch_started=0
...
cleanup() {
  ...
  if [[ -n "$dry" && "$dry" == /tmp/bizflow-pages-check.* ]]; then rm -rf -- "$dry"; fi
...
dry=$(mktemp -d /tmp/bizflow-pages-check.XXXXXX)
cp "$backup/business-control.db" "$dry/check.db"
mkdir "$dry/uploads"
chown -R business-control:business-control "$dry"
runuser -u business-control -- env BUSINESS_ADDRESS=127.0.0.1:18539 BUSINESS_DATABASE_PATH="$dry/check.db" BUSINESS_UPLOAD_PATH="$dry/uploads" "$release/business-control" > "$dry/server.log" 2>&1 &
dry_pid=$!
for attempt in $(seq 1 30); do
  if curl -fsS http://127.0.0.1:18539/api/health > "$dry/health.json" 2>/dev/null; then break; fi
  test "$attempt" -lt 30
  sleep 1
done
curl -fsS http://127.0.0.1:18539/ > "$dry/index.html"
grep -q '20260902-all-pages-1' "$dry/index.html"
...
grep -q '20260902-all-pages-1' "$backup/local-index.html"
grep -q '20260902-all-pages-1' "$backup/public-index.html"
